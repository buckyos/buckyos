...
3. **CLI 默认输出 JSON**
   stdout 默认是 `AgentToolResult` 协议（带 `agent_tool_protocol: "1"`），方便 Runtime 的 WorkLog 压缩渲染。只有少数命令（例如 `read_file` 的 pipe 模式）才允许在明确的条件下退化成纯文本。
...
- 解析侧只接受 `agent_tool_protocol: "1"`，不再接受历史 `is_agent_tool` 别名；Rust 内部结构体也只保留 `agent_tool_protocol` 字段
...
2. **业务结果走 `AgentToolResult`**，至少填 `cmd_name` / `status` / `summary` / `detail`；`AgentToolResult::from_details` 会写入 `agent_tool_protocol: "1"`。回顾一下四个字段对应的压缩档（详见 [`agent_tool_result_protocol.md`](agent_tool_result_protocol.md) §4）：`title` 决定 `Min` 档单行；`summary` 决定 `Mini` / `Medium` 档进入 prompt 的内容；`cmd_name + cmd_args + detail|output` 决定 `Full` 档。
...
- 用 stdout 的内容启发式猜测「这是不是 AgentTool 输出」，而不是看 `agent_tool_protocol`
