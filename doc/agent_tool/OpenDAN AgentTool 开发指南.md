...
- 知道 AgentTool 的预装清单与按 session 扩展机制是如何实现的
...
3. **CLI 默认输出 JSON**
   stdout 默认是 `AgentToolResult` 协议（带 `agent_tool_protocol: "1"`，等价于历史的 `is_agent_tool=true`），方便 Runtime 的 WorkLog 压缩渲染。只有少数命令（例如 `read_file` 的 pipe 模式）才允许在明确的条件下退化成纯文本。
...
OpenDAN 内置 AgentTool 的部署形态固定为「一个 `agent_tool` 主二进制 + 多个命令别名」：
...
新增一个内置工具时，只在 `cli.rs::TOOL_NAMES` 里把工具名注册进去，dispatch 自然生效；不要再单独构造一个 bin。

> busybox 模式只约束 OpenDAN 自带的内置 AgentTool。第三方工具不受该约束：第三方可以是独立可执行文件，也可以反过来把 `read_file` / `write_file` 等内置命令名软链到它自己的二进制上去——session 软链接目录由 `sync_session_tool_links` 按 session 当前可见工具集刷新（见 §2.2），同名 link 谁出现在 `PATH` 前面谁生效。第三方工具只要 stdout 仍然按 §4 输出 `AgentToolResult`，对 Runtime 而言就是等价的。

### 2.2 Runtime 给 tmux 准备的环境

`exec_bash` 在每次进入 session 之前都会刷新一份「该 session 专属」的工具目录和环境变量（参考 `agent_bash.rs::build_exec_env_vars` / `prepare_session_tool_env`）。这一步每次执行都会做，是为了让 OpenDAN Runtime 升级、新工具上线、session 卸载工具等场景下软链接目录始终与当前 session 的可见工具集一致：

- 软链接目录：
  `<buckyos_root>/agent_bash_tools/<sanitized_agent_id>/<session_id>/`
  里面是该 session 当前可见的所有 AgentTool 命令别名，由 `sync_session_tool_links` 按 `default_agent_cli_tool_names()` 与 session 当前 `loaded_tools` 的并集差异化刷新——多余的链接会被清掉、缺失的链接会被补上。这就是 AgentTool 系统的「预装 + 按 session 扩展」机制：默认集来自 `default_agent_cli_tool_names()`，session 在运行期通过 `loaded_tools` 增量扩展。
- 注入到 tmux pane 的环境变量（用于在 CLI 进程里还原 `SessionRuntimeContext`）。原则上**只要 `OPENDAN_AGENT_ENV` + `OPENDAN_SESSION_ID` 这两个根 + session 标识就足以定位所有资源**，其余变量是给 prompt / worklog / 调试做溯源的辅助标签；真正的目录定位优先用 PWD/CWD 与 RootFS 推导，避免出现「没有设置环境变量就无法工作」的情况（参考 `cli::CliRuntimeEnv::from_process` 里 `OPENDAN_AGENT_ENV` 缺失时回退到 `current_dir` 的写法）：
...
> TODO（用户工具覆盖）：进程内调度还需要补一步——在 `support_bash` 命中前，先检查 session 软链接目录是否存在指向用户自定义工具二进制的同名 link。若存在，则不再走 in-process 内置实现，而是 fork 到该用户工具的 CLI，让 bash 路径与 in-process 路径都对齐到用户覆盖后的实现上。当前实现里 in-process 路径直接命中内置工具，不感知用户 link，需要后续补齐。
...
- 工具实现里**不允许**回调 OpenDAN runtime 的内部组件；如果需要操作 `todo` / `worklog`，请通过 `agent_tool` 自己提供的 lib（例如 `TodoTool::new(...)`）。
  边界原则是「OpenDAN 依赖 AgentTool」，反向不成立。
...
stdout 默认是一段单行 JSON（结构定义见 `lib.rs::AgentToolResult` 与 [`agent_tool_result_protocol.md`](agent_tool_result_protocol.md)），最少字段：
...
- `agent_tool_protocol` **必须**显式带上（当前协议版本是字符串 `"1"`），它同时承担「这是 AgentTool 协议结果」+「schema 版本号」两个职责，避免 `exec_bash` 把碰巧像 JSON 的普通 stdout 误判成 AgentTool
- 兼容：历史输出里的 `is_agent_tool: true` 等价于 `agent_tool_protocol: "1"`，解析侧两种都接受；新输出统一写 `agent_tool_protocol`。Rust 内部结构体仍保留 `is_agent_tool: bool` 字段以及 `with_is_agent_tool(true)` 构造器，序列化时映射为 `agent_tool_protocol: "1"`
- `status` ∈ `success | error | pending`
- `summary` 永远填，给人读、给 prompt 压缩用——`Mini` / `Medium` 档下它是唯一会被渲染进 prompt 的内容
- `detail` 是结构化业务结果；普通 bash 的 `output` 字段对 builtin tool 默认不填，因为 builtin tool 的压缩档展示不依赖 `output`
- exit_code：`success → 0`，`error → 非 0`（默认 `1`），`pending → 非 0`（约定的「未完成」码）；对应 `cli_exit_code_for_error` 与 `CLI_EXIT_*` 常量
- 出错时不要去掉 `agent_tool_protocol`：错误也是 AgentTool 的输出
...
`pending_reason` 仅取 `long_running | user_approval | wait_for_install`（历史值 `external_callback` 仅做兼容别名解析为 `wait_for_install`，新输出不要再用）。Agent loop 看到 `pending` 后会按 `check_after` 秒数调用 `check_task <task_id>` / `cancel_task <task_id>` 跟进。
...
2. **业务结果走 `AgentToolResult`**，至少填 `cmd_name` / `status` / `summary` / `detail`，并显式 `with_is_agent_tool(true)`（Rust 构造器名沿用，序列化为 `agent_tool_protocol: "1"`）。回顾一下四个字段对应的压缩档（详见 [`agent_tool_result_protocol.md`](agent_tool_result_protocol.md) §4）：`cmd_name` / `cmd_args` 决定 `Min` 档单行；`summary` 决定 `Mini` / `Medium` 档进入 prompt 的内容；`detail` 决定 `Full` 档 ```` ```result ```` 块；普通 bash 的 `output` 决定各档 excerpt——builtin tool 默认不依赖 `output` 渲染。
...
6. **更新文档**：把工具加进 [builtin_agent_tools.md](builtin_agent_tools.md) 的「当前 builtin tools 一览」+ 单独一节写输入字段、`cmd_name` / `summary` / `detail` 的输出约定。
7. **测试**：
...
- 用 stdout 的内容启发式猜测「这是不是 AgentTool 输出」，而不是看 `agent_tool_protocol`（或兼容别名 `is_agent_tool=true`）
...
- [readme.md](readme.md)：CLI 化与异步执行模型的设计动机
- [agent_tool_result_protocol.md](agent_tool_result_protocol.md)：`AgentToolResult` 字段定义、压缩档渲染规则
