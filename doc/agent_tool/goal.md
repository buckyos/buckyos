...
1）将AgentToolResutl升级到下面协议 

```json
{
  "status": "success|error|pending",
  "summary": "human readable summary",
  "output": "",//全部的输出，统一stdout和stderr,就是tmux上按时间的混合显示
  "partial_output":"",//部分输出
  "return_code" : 0,//如果bash命令有可以填
  "cmd_name":"",//命令名
  "cmd_args":"",//命令的参数列表
  "task_id": "optional",
...
2）agent_bash执行时，直接返回AgentToolResult
内部工具实现时，默认返回AgentToolResult，并将encode后的json（不包含cmd_name,cmd_args)显示在bash中（内部工具通常只有summary,status,output这几个字段)
ExecBash在处理时，可以根据执行输出是否是json,主动转换成AgentToolResult。否则用默认逻辑拼接得到Agent ToolResult

3) Prompt / WorkLog 统一使用AgentToolResult
