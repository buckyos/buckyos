...
提供所有业务模块共用的长任务观察、操作审计、结构化日志查询和脱敏诊断导出。业务模块只
...
审计事件是 append-only 事实，稳定 ID 为 `audit_id`。记录 actor、action、resource、trace、
outcome、可选错误码和已经脱敏的 details。普通用户只能看到自己的事件；Admin/Root 可以查询
Zone 范围。持久格式见 [task-audit-durable-schema.md](../task-audit-durable-schema.md)。
...
| `audit query` | actor/action/resource/trace/since/until/cursor/limit | privileged/read | sync | 按 actor scope 分页查询 |
...
Task/audit 的 `since/until` 接受 RFC 3339 或 Unix milliseconds，发送给 TaskManager 时统一为 milliseconds；
...
- audit own scope 是 read，Zone scope 和 actor 指定为 privileged；服务端不能相信客户端自报 actor；
...
- stdout、stderr、error details、audit details 和 manifest 不得包含下载 token或凭证。
...
| audit query | `/kapi/control-panel`: `audit.query`；handler 再调用 TaskManager `list_audit_events` |
| audit append | zone-trusted service 调用 TaskManager `append_audit_event` |
...
- Task、TaskEvent 和 AuditEvent 使用 TaskManager 平台 RDB instance；
- AuditEvent 是 append-only，不允许 update/delete API；
...
- Beta 2.2 是 breaking change，audit schema 初始版本不做旧数据兼容。
...
- audit query 覆盖普通用户 actor 裁剪、Admin filter、cursor 和 trace filter；
...
- `command describe` 能发现全部 11 条命令和固定 schema。
...
facade、审计表、脱敏和 diagnostic bundle。
