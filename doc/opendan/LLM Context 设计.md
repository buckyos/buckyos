...
§6.2 / §6.3 / §6.4 描述的 resume 路径都假设"snapshot 是因为某个挂起态 outcome 而产生的"——pending tool 等结果回填、wait input 等人，等等。但 L4 持久化层（OneShot 在本地目录 / workflow runtime 在 storage）想做更细粒度的崩溃恢复时，需要在**没有挂起态信号**的轮间也落盘 snapshot：
...
1. **L4 必须有能力区分"崩在挂起态"与"崩在运行中"两条路径**：前者用 `ResumeFill::{ToolResults, HumanInput, RewrittenHistory}`，后者用 `ResumeFromMidRun`。waist 在 resume 里会做一致性校验拦截误用。L4 的元数据（例：`LocalLLMContext::RunMetaState.last_suspend_kind`）就是用来记这条信息的。
...
`LocalLLMContext` 还顺手承担了**"OneShot 的硬默认值"**：75% context yield ratio、`ErrorMode::Suspend`、自动注入 `ResumeFill::RewrittenHistory` 的压缩 loop。这些都是 L4 层政策，**不进 waist**——同样的 waist 接口下，`LLMAgentContext` 完全可以用不同的默认值（参见 §3.11 末段对各 L4 默认值的描述）。
