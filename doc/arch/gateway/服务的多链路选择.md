...
>
> **术语对照**：本文中的"应用层调度器"是从 cyfs-gateway 视角对调度角色的描述——对 cyfs-gateway 来说，凡是不在 gateway 进程内、负责生产 / 刷新 URL 集合的组件都属于"应用层"。在 BuckyOS 实际架构里，承担这个角色的就是 **kernel scheduler**（在 Node 上为 service 生产 routes）和 **buckyos-runtime**（在 Client Device 上为客户端访问做一次性链路优化），并不是另一个独立组件。看到"应用层调度器"时按此对应即可。
...
### 6.6 三层 cache epoch 边界

为了避免 "Selector 稳定选择" 与 "Tunnel 内部每次 OpenStream 重新竞速" 的语义打架，三层各自的 cache 作用域必须清晰：

- **IP 层**：RTT、上一次成功 IP 等只在单个 OpenStream 周期内生效；这层的"重新选择"由 250ms 阈值触发，不影响上层 URL 排序。
- **URL 层**：tunnel_mgr URL history 跨连接复用，executor 在每次 attempt 结束时按 §6.7 的口径回写。这层数据是 Selector 做 RTT / least-time 重排时的输入。
- **Selector 层**：在一个调度刷新 epoch 内（scheduler 周期 / runtime 一次性 probe）保持稳定。Selector 不直接观察 IP 级抖动；URL 级排序的变化也只在下一次配置刷新或显式 Probe 调用时才被采纳。

这样请求级别的 IP 抖动不会反向震荡到 Selector 的输出，又能让真实的 URL 健康变化在下一个 epoch 被吸收。
...
这个能力类似于 IP 排序，但对象从 IP 扩展为 Tunnel URL。当前 cyfs-gateway 已经实现了对应能力：`tunnel_mgr` 维护 `normalized_url -> TunnelUrlHistory`，对外提供批量 URL 状态查询；`forward` 命令的 group 形态在执行阶段消费这份历史做 RTT / least-time 重排（见 cyfs-gateway `doc/forward机制升级需求.md` 阶段 4 的 `apply_least_time_via_tunnel_mgr`）。本文中提到"Gateway URL Probe"或"测速排序基础设施"指的就是这套能力。
...
Gateway 负责将语义服务映射到一组 Tunnel URL，并执行标准 Selector 与 Failover 策略。这一层在当前实现中由 cyfs-gateway 的 `ForwardPlan` + `forward` group 命令承担：process-chain 把 `ServiceInfo.selector × ROUTES[node_id]` 展开成 primary / backup peer map，传给 group forward；executor 在连接阶段按 `next_upstream` 重试，session 建立后不再透明切换（详见 cyfs-gateway `doc/forward机制升级需求.md`）。
