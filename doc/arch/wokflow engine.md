...
**Workflow 编排器（Expr Tree Evaluator）**：持有整棵 Expr Tree，负责编排语义——哪些节点可以并行、branch 走哪条路、for_each 怎么展开、什么时候等人类确认。当某个节点的前置依赖都满足时，编排器构造一个 Thunk Object 投递给调度器。调度器返回结果后，编排器更新 Expr Tree 的状态，决定下一步投递什么。

**Thunk 调度器（Thunk Dispatcher）**：只做一件事——收到一个 Thunk，检查参数是否就绪（`check_param_is_ready`），就绪则找一个物理节点执行，执行完把结果写入 Named Object。调度器不知道 Thunk 来自哪棵 Expr Tree、不知道 Thunk 之间的依赖关系、不知道这个 Run 属于谁的什么业务。
...
| 人类交互（暂停、审核、回退） | 编排器 | 不管 Thunk 执行细节 |
| 参数就绪检查、节点选择、执行 | 调度器 | 不管 Thunk 之间的关系 |
| 结果写入 Named Object | 调度器 | 不管谁消费结果 |
| 全局 Memoization（cache） | Named Object 层 | 编排器投递前查，调度器无感 |

**关键性质**：两个团队可以完全独立迭代。调度器只认 Thunk Object 这一种类型，不管 Workflow DSL 怎么演进。编排器只管投递 Thunk 和接收结果，不管调度器是单机还是分布式。

### 2.5 DSL → Expr Tree → Thunk：编译而非解析
...
Expr Tree 是引擎内部唯一的执行表示。调度器只认 Thunk Object（Expr Tree 的单个叶子节点在投递时的具体化形式）。
...
|----------|---------------|------|
| `autonomous` Step | `Apply` | fun_id = hash(executor)，params 从 input 中的 Reference 解析 |
...
### 3.11 Thunk Object（调度器原语）

Thunk Object 是编排器投递给调度器的最小执行单元。它是 Expr Tree 的一个 `Apply` 节点在投递时的**具体化形式**——所有 Reference 已被解析为实际值，所有编排语义（依赖、分支、并行）已被编排器处理完毕。
...
调度器收到 Thunk 后执行参数就绪检查。检查逻辑极其简单：
...
编排器（Workflow Engine）通过 Thunk Object（见 3.11）与调度器交互。本节定义交互流程。

#### 4.7.1 投递流程（编排器 → 调度器）

当编排器判断某个 `Apply` 节点的所有依赖已满足时：

1. **解析 Reference**：将 Expr Tree 节点的 `params` 中所有 `Reference` 替换为实际值（从 `node_outputs` 中取）。
2. **查 Cache**（仅 `idempotent: true`）：用 `fun_id + hash(resolved_params)` 查询 Named Object。命中则直接将结果写入 `node_outputs`，不投递给调度器。
3. **构造 Thunk Object**：将 `fun_id`、解析后的参数、资源需求、元信息（run_id、node_id、attempt）打包为 Thunk Object。
4. **写入 Named Store**：将 Thunk Object 写入 Named Store，获得 `thunk_obj_id`。
5. **投递**：调用 `schedule_thunk(thunk_obj_id)`。
...
**只有 `Apply` 节点会产生 Thunk 投递给调度器。** `Match`、`Par`、`Map` 是纯编排逻辑，在编排器内部就地处理。`Await` 是人类交互，编排器自己管理暂停和恢复。
    ...
    while True:
        ...
        for node in actionable:
            ...
            match node:
                ...
                case Apply(fun_id, params, output_mode, idempotent, guards):
                    # 幂等节点先查 cache
                    if idempotent:
...
1. **只有 Apply 产生 Thunk。** Match / Par / Map / Await 全部在编排器内部处理，调度器看不到也不需要看到。
...
- FuncObject schema（funid + param_hashes + result）
- ThunkObject schema（见 3.11）
- ResultObject schema
- Named Store 存储接口 + cache 查询接口

**Step 3：FuncType 定义 + node-executor（依赖 Step 2）**

- KB 场景需要的 FuncType：`skill/fs-scanner`、`agent/mia`、`skill/kb-ingest`、`skill/kb-index-builder`
- 本地 node-executor 实现
...
3. 编排器能按 Expr Tree 的依赖关系**正确地向调度器投递 Thunk**，幂等节点命中 Named Object cache 时跳过投递。
...
| **Thunk Object** | 编排器投递给调度器的最小执行单元。Expr Tree 的 Apply 节点在投递时的具体化形式 |
...
| **Executor** | 物理节点上实际执行 Thunk 的运行时。接收 fun_id + params，返回 result |
| **FunId** | 函数的内容寻址标识（executor 代码的 hash）。相同代码永远产生相同 FunId |
| **Named Object** | 全局内容寻址存储。`funid(hash(params)) → result` 的全局 memoization |
| **编排器（Orchestrator）** | 持有 Expr Tree，管理编排语义（依赖、分支、并行、人类交互），在正确时机投递 Thunk |
| **调度器（Thunk Dispatcher）** | 收到 Thunk，检查参数就绪，选择节点，执行，返回结果。不理解 Workflow 语义 |
