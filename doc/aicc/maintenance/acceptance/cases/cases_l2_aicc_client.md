...
| `provider.update` | Provider Instance、enabled/endpoint/credential/Profile/Adapter/discovery patch | revision CAS 写入、enable/disable 与实例替换生命周期、公共 client schema | 实例不存在、revision 冲突、非法 patch、旧 generation 迟到写入 |
