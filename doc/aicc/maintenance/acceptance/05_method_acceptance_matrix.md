...
| `provider.update` | Provider Instance、enabled/base_url/credential/Profile/Adapter/discovery patch | revision CAS 写入；enable/disable 与实例替换生命周期正确 | 实例不存在、revision 冲突、非法 patch、停止失败、旧 generation 迟到写入 |
