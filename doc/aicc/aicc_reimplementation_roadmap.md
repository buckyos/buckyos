...
实现记录：公共 API 已改为逐方法强类型 request/response，Canonical IR、稳定错误边界和 client/handler/server dispatch 已落地；Workflow、OpenDAN、Agent Tool、Control Panel 已迁移到类型化调用，管理面 wire method 只保留 `service.reload_settings`。WP-16 所需的 `provider.catalog`、`protocol_adapter.list`、`provider.validate/add/delete/refresh_models`、`usage.query`、`trace.query`、`routing.get` 和 `routing.update` 均提供 canonical request/response、Client、Handler 和 kRPC dispatch，不需要 service 层私建协议；`routing.update.provider_weights` 采用完整替换语义，空 map 表示清空，并使用 settings revision CAS。`provider.list` 返回带 `settings_revision` 的脱敏强类型 `ProviderInstanceView`，可明确表达 disabled/未加载的 inventory 与 health，settings CAS 冲突使用 `settings_revision_conflict` 和固定 revision details；`DriverMetadataUpdateView` 同时公开 NDN `metadata_target_seq` 和逐 Provider 的 `metadata_applied_seq`。WP-14 usage 契约要求事件显式携带 `user_id`、canonical typed `method` 和 `provider_instance_name`，查询可按三者过滤和分组，显式时间范围采用半开区间 `[start_time_ms, end_time_ms)`；usage RDB schema version 已提升为 6，并用可选 `trace_id` 关联路由、任务、事件和 usage。WP-09 quota 与策略金额统一使用严格的 `Money { amount, currency }` DTO，wire 字段为 `remaining_cost`、`max_estimated_cost` 和 `max_cost`，不再用字段名隐含 USD。验收覆盖 `buckyos-api`/`aicc` 单测、受影响 crate 全 target check、workspace test compile、协议残留扫描和格式检查；AICC 严格 clippy 被仓库范围外的既有告警基线阻断。

WP-18 execution mode 契约补充：`route.resolve`、全部 typed inference request 和两个 Helper request 统一使用严格枚举 `execution_mode = immediate | stream`，缺失时默认为 `immediate`；公共 `immediate` 表示调用方不请求流式返回，native-only operation 在服务内部进入持久异步 task 生命周期并可先返回 `running`，不向调用方暴露内部 `native_task`。公共错误码 `unsupported_execution_mode` 表示所请求的公开模式无法由 Adapter/operation 实现，例如对 native-only operation 请求 `stream`；禁止把 `stream` 静默降级为非流式执行。
...
| WP-08C（行为与配置化完成） | Gemini | Gemini Interactions |
...
WP-08A 实现记录：OpenAI builtin 装配位于 `src/frame/aicc/src/provider/builtin/openai.rs`，Rust 只保留稳定 `openai` Profile ID、Bearer 鉴权行为、官方 Models API discovery 和基础 `openai-responses` codec 复用，并暴露三类 builtin catalog 文件集。显示名称、默认 `base_url`、Adapter、credential 与 region/workspace/account 声明由 `driver_metadata/known-providers/openai.known-provider.json` 提供；Model Driver 限定及 LLM、embedding、image、audio、video operation 映射由 `driver_metadata/providers/openai.provider.json` 提供；OpenAI 原厂发布模型的 API type、能力、上下文/输出限制、按 token 价格和 reasoning variant 由 `driver_metadata/models/openai.model.json` 提供，未知模型保持 conservative fallback。配置事实依据为 OpenAI 官方 [模型目录](https://developers.openai.com/api/docs/models)、[GPT-5.6 Sol](https://developers.openai.com/api/docs/models/gpt-5.6-sol)、[GPT-5.6 Terra](https://developers.openai.com/api/docs/models/gpt-5.6-terra)、[GPT-5.6 Luna](https://developers.openai.com/api/docs/models/gpt-5.6-luna)、[GPT Image 2](https://developers.openai.com/api/docs/models/gpt-image-2)、[Embeddings API](https://developers.openai.com/api/reference/ruby/resources/embeddings/methods/create)、[GPT-4o mini TTS](https://developers.openai.com/api/docs/models/gpt-4o-mini-tts) 和 [GPT Transcribe](https://developers.openai.com/api/docs/models/gpt-transcribe) 文档。Provider Rules 的 revision 不再由 Rust 调用参数改写，模型名、operation、能力、价格和推理档位均不在 builtin 代码中补表；OpenAI 没有额外 wire 差异，因此不创建空 dialect，也未修改基础 codec。三类 catalog 已通过 WP-15 `MetadataSources` 的 builtin 来源选择入口构建同一 `CatalogSnapshot` 和真实 inventory，5 个 WP-08A 聚焦测试及 AICC 全量 273 个测试通过，`cargo check -p aicc --all-targets`、Rust 格式和 diff 检查通过；stable clippy 在仅豁免 WP-08E 现有 `redundant_closure` 与 Resource 模块既有 `manual_is_multiple_of` 后以 `-D warnings` 通过，未新增依赖。`buckyos-build.py --skip-web` 仍因缺少四个 `BUCKYOS_SDK_TOOL_*` 不可变构建输入而无法执行；后续服务启动装配由 WP-16 统一完成，真实 OpenAI API 与 T1/T1.5/T2 验收按 WP-18 集成阶段执行。

WP-08B 实现记录：Claude 与 MiniMax builtin 装配分别落在 `src/frame/aicc/src/provider/builtin/claude.rs` 和 `src/frame/aicc/src/provider/builtin/minimax.rs`，Rust 仅保留稳定 Profile ID、分页 Models API discovery、连接解析及无法声明化的 Messages wire 差异。显示名称、默认 `base_url`、Adapter、`x-api-key` credential 和 region/workspace/account 声明由 `driver_metadata/known-providers/{claude,minimax}.known-provider.json` 分厂商提供，MiniMax global/china 地址映射也由该配置提供；operation、Model Driver 绑定、请求默认值及不支持参数移除由 `driver_metadata/providers/{claude,minimax}.provider.json` 提供。Claude 与 MiniMax 均为模型原厂，其当前模型 ID、能力、token 限制和可安全表达的固定价格分别由 `driver_metadata/models/anthropic.model.json` 与 `driver_metadata/models/minimax.model.json` 提供；MiniMax M3 的分段价格无法由当前 schema 准确表达，因此未写入固定价格。配置事实依据为 Anthropic 官方 [Models overview](https://platform.claude.com/docs/en/about-claude/models/overview) 与 [List Models API](https://platform.claude.com/docs/en/api/models/list)，以及 MiniMax 官方 [Anthropic-compatible API](https://platform.minimax.io/docs/api-reference/text-chat-anthropic)、[List Models API](https://platform.minimax.io/docs/api-reference/models/anthropic/list-models) 和 [Pay-as-you-go pricing](https://platform.minimax.io/docs/guides/pricing-paygo)。动态 discovery 仅返回远端模型身份和可用性，模型能力、operation 和价格统一由 WP-15 catalog snapshot 合并。Claude 直接注册未修改的 `claude-messages` 基础 codec；MiniMax 的 `minimax-messages` 显式声明 `base_adapter_id: claude-messages`，只处理温度上限、`base_resp` 错误及 ProviderState namespace，其余请求规则和 request/response/SSE 语义分别交给 Provider Rules 与基础 codec。配置合同、连接、discovery 和 dialect 定向测试覆盖上述边界，未新增依赖。

WP-08C 实现记录：Gemini builtin 装配已落在 `src/frame/aicc/src/provider/builtin/gemini.rs`，只保留稳定 `gemini` Profile ID、named-header credential 行为、Models API discovery 和 `gemini-interactions` codec 注册，并为 WP-15 暴露三类 builtin catalog 文件集。显示名称、默认 `base_url`、Adapter、credential/instance field 声明由 `driver_metadata/known-providers/gemini.known-provider.json` 提供；Model Driver 限定和 LLM、vision、embedding、image、audio、video operation 映射由 `driver_metadata/providers/gemini.provider.json` 提供；Google 原厂发布模型的 API type、能力、context/output token 限制及 embedding 维度由 `driver_metadata/models/gemini.model.json` 提供。配置事实依据为 Google 官方 [Models API](https://ai.google.dev/api/models)、[Gemini models](https://ai.google.dev/gemini-api/docs/models)、[Interactions API](https://ai.google.dev/api/interactions-api-v1)、[API key](https://ai.google.dev/gemini-api/docs/api-key)、[Embeddings](https://ai.google.dev/gemini-api/docs/embeddings) 和 [Image generation](https://ai.google.dev/gemini-api/docs/image-generation) 文档。discovery 仅保留官方返回的原始 model/base model identity，不再在 Rust 中把 `supportedGenerationMethods` 映射为 AICC operation，重复模型和 page token 均拒绝，inventory revision 由模型身份稳定计算；动态 discovery 不声明价格或扩张 Model Driver 能力。三类 builtin catalog 已通过 WP-15 `MetadataSources` 的 builtin 来源进入同一个 `CatalogSnapshot`，并构建真实 inventory；6 个 WP-08C 定向测试及 AICC 全量 274 个测试通过，`cargo check -p aicc --all-targets` 和 diff 检查通过，未新增依赖。完整构建仍缺少四个 `BUCKYOS_SDK_TOOL_*` 输入；真实 Gemini API 与 T1/T1.5/T2 验收按 WP-18 集成阶段执行。

WP-08D 实现记录（已完成，提交 `105ecde9`）：OpenRouter、Kimi 和 GLM builtin 装配分别落在 `src/frame/aicc/src/provider/builtin/openrouter.rs`、`kimi.rs` 和 `glm.rs`，三个窄 dialect 继续只处理 routing、thinking/reasoning、cache、partial、tool stream 与 ProviderState 等不可安全声明化的 wire 差异，基础请求、响应、SSE 和错误委托 WP-06D codec。显示名称、默认 `base_url`、Adapter、credential 与 region/workspace/account 声明均由 `driver_metadata/known-providers/{openrouter,kimi,glm}.known-provider.json` 提供，operation、origin mapping 和请求参数规则由 `driver_metadata/providers/{openrouter,kimi,glm}.provider.json` 提供；Rust 不再构造生产用 Known Provider、Provider Rules 或 Model Driver。OpenRouter 依据官方 [Models API](https://openrouter.ai/docs/guides/overview/models) 和 [Provider Routing](https://openrouter.ai/docs/guides/routing/provider-selection) 定位为聚合 Provider，仅配置 Provider 信息并继续动态发现模型、能力和价格，不创建 OpenRouter Model Driver；Kimi 与 GLM 是模型原始发布厂商，依据 Kimi 官方 [模型列表](https://platform.kimi.ai/docs/models)、[Kimi K3](https://platform.kimi.ai/docs/guide/kimi-k3-quickstart) 及 Z.ai 官方 [GLM-5.3](https://docs.z.ai/guides/llm/glm-5.3)、[GLM-5.2](https://docs.z.ai/guides/llm/glm-5.2) 文档，在 `driver_metadata/models/{kimi,glm}.model.json` 配置当前模型 ID、上下文/输出限制和公开能力，并通过 Provider Rules 表达 Kimi K3 固定采样参数剔除及 GLM-5.3 强制 thinking/default reasoning effort。八份 catalog 已通过 WP-15 builtin metadata source 构建同一 `CatalogSnapshot`，明确验证 OpenRouter 无 Model Driver、Kimi/GLM 有原厂 Model Driver；WP-08D builtin 测试 44/44、AICC 全量测试 276/276 和 `cargo check -p aicc --all-targets` 通过，stable `rustfmt --check` 通过，未新增依赖。默认 Rust 1.95 工具链缺少 `rustfmt`/`clippy` 组件，因此该工具链的格式和 clippy 命令未执行；完整 `buckyos-build.py --skip-web` 仍需提供四个 `BUCKYOS_SDK_TOOL_*` 不可变构建输入后复验。

WP-08E 实现记录：DeepSeek、豆包和 Qwen 作为 OpenAI Responses 兼容 Provider，builtin 装配已落在 `src/frame/aicc/src/provider/builtin/openai_responses_compatible.rs`；该模块按协议兼容关系组织，可继续接入满足相同契约的 Provider。Rust 仅保留稳定 Profile ID、DeepSeek Models API discovery、catalog-only discovery、连接模板执行和不可声明化的 Responses wire 差异。显示名称、默认 `base_url`、Adapter、Bearer credential 与 region/workspace/account 声明由 `driver_metadata/known-providers/{deepseek,doubao,qwen}.known-provider.json` 分厂商提供；operation、Model Driver 限定、请求参数收窄与 reasoning variant 由 `driver_metadata/providers/{deepseek,doubao,qwen}.provider.json` 分厂商提供；三家均为模型原厂，其已确认的模型 ID/系列、能力与 token 限制由 `driver_metadata/models/{deepseek,doubao,qwen}.model.json` 分厂商提供，未知模型保持 conservative fallback。配置事实依据为 DeepSeek 官方 [Models & Pricing](https://api-docs.deepseek.com/quick_start/pricing/)、[Responses API](https://api-docs.deepseek.com/api/create-response/) 与 [Thinking Mode](https://api-docs.deepseek.com/guides/thinking_mode/)，火山方舟官方 [Responses API](https://www.volcengine.com/docs/82379/1795150) 与 [豆包模型产品页](https://www.volcengine.com/product/doubao/)，以及阿里云百炼官方 [Qwen Responses API](https://help.aliyun.com/zh/model-studio/qwen-api-via-openai-responses) 与 [文本生成模型列表](https://help.aliyun.com/zh/model-studio/text-generation-model)。动态分时价格及无法由当前 schema 按 region 安全表达的 variant 没有写入配置。`src/frame/aicc/src/protocol/derived_responses.rs` 不再保存 Provider 参数黑名单，只保留 Qwen session-cache header 与三家 ProviderState namespace 的最小 wire 行为，其余 request/response/SSE/error 继续委托 WP-06A 基础 codec。九份 builtin catalog 已通过 WP-15 `MetadataSources` 的 builtin source 选择入口构建同一 `CatalogSnapshot` 和真实 inventory，WP-08E 9 个定向测试及 AICC 全量 273 个测试通过，`cargo check -p aicc --all-targets`、Rust 文件格式和 diff 检查通过，未新增依赖；后续服务启动装配由 WP-16 统一完成，豆包/Qwen 原生媒体 operation 按本文建议批次 4 另行实施。

WP-08F 实现记录（已完成）：fal builtin 装配位于 `src/frame/aicc/src/provider/builtin/fal.rs`，Rust 仅保留稳定 `fal` Profile ID、catalog-only discovery 和无法声明化的 Queue 状态机行为。显示名称、默认 `https://queue.fal.run`、`fal-queue` Adapter、`Authorization: Key` credential 及 region/workspace/account 声明由 `driver_metadata/known-providers/fal.known-provider.json` 提供；image/audio/video API type 到 `queue.submit` 的 operation 映射由 `driver_metadata/providers/fal.provider.json` 提供。当前尚未接入相应原厂 Provider 的 `fal-ai/esrgan`、`fal-ai/imageutils/rembg`、`fal-ai/deepfilternet3` 和 `fal-ai/video-upscaler` 暂归入 `driver_metadata/models/fal.model.json`，由 Model Driver 声明稳定能力、逻辑挂载和延迟。catalog-only Provider 从 Provider Rules 的精确模型列表构建内置 default inventory，Provider Instance 可用显式 discovery inventory 覆盖。`src/frame/aicc/src/protocol/fal_queue.rs` 继续实现独立 `fal-queue / queue.submit` native-task Adapter，覆盖媒体 typed request lowering、submit/status/result/cancel、生命周期、官方错误、路径校验、资源归一化及 artifact 输出，并复用 WP-05 polling/deadline/backoff/cancel contract。fal 当前按 compute-second、audio-second 或 megapixel 等不同维度计价，现有静态 Pricing schema 无法准确表达的价格不写入 Provider Rules。

WP-08G 实现记录：SN 扩展 Provider 装配已落在 `src/frame/aicc/src/provider/builtin/sn.rs`，提供稳定 `sn` Profile；独立 `sn.known-provider.json` 配置 `BuckyOS SN` 显示信息、默认 `https://sn.buckyos.ai/api/v1/ai`、region/workspace/account schema 及互斥的 `api_key`/`dynamic_login` credential schema，独立 `sn.provider.json` 配置 `responses.create` operation 和能力收窄。根据 [SN 官方产品说明](https://sn.buckyos.ai/)，SN LLM Loader 是可访问多家云模型的统一 AI 网关而非模型原厂，因此不提供 `sn.model.json`，Provider Rules 也不硬编码单一 Model Driver；模型归属按 discovery 返回的原始模型名在当前 Model Driver catalog 中解析。SN discovery 使用带 Bearer credential 的 `/models` 机器接口，仅把真实返回的模型作为 LLM inventory。`sn-openai` 声明 `base_adapter_id: openai-responses`，仅覆盖 credential resolution 并通过 WP-05 派生注册直接委托基础 Responses codec；动态登录复用 BuckyOS device-token 登录 API，按 TTL 在内存缓存 token，同实例并发刷新合并，替换/删除时可失效，认证错误及 Debug 输出不暴露 token。配置通过 WP-15 `MetadataSources` 的 builtin source 选择入口构建 catalog snapshot，Rust builtin 不再构造 SN 的 Known Provider 或 Provider Rules 第二真相源；配置合同测试同时验证 SN catalog 不包含 Model Driver。SN 定向测试 8/8、AICC 全量测试 276/276、`cargo check -p aicc --all-targets`、Rust 格式、JSON 语法和 diff 检查通过；stable clippy 在仅豁免 WP-08E 既有 `redundant_closure` 后以 `-D warnings` 通过，未新增依赖。
...
- [x] fail closed 处理 privacy、trust 和 credential 等安全真相源读取失败；quota 不可得保留为 `unknown`；
- [x] 跨租户、跨应用和 credential scope 测试。

完成标准：实现位于 routing 内部策略层而非独立顶层模块；Router 把策略判定作为确定的 hard filter 输入，不自行读取 quota 或安全配置。

实现记录：Policy/Security 小组在 `src/frame/aicc/src/routing/policy.rs` 实现 system、user、app、session、request 五级字段合并和 locked 冲突拒绝，复用统一 MatchRule 编译 Provider allow/block 规则；策略引擎以只读 trust、credential scope 和 quota source 视图判定 trusted local、隐私、单次成本、请求额度与剩余预算，只返回 hard filter 原因和 local-first 偏好，不实现模型 admission 或候选评分。`quota.query` 通过公共 DTO 返回调用者作用域视图；privacy、trust 和 credential 等安全真相源失败继续 fail closed，quota 缺失、不支持、查询失败或未知则返回 `unknown` 并保留候选，只有明确耗尽或超过明确配置的预算才拒绝对应候选。

补充 Service 集成契约：WP-09 提供对象安全的异步 `QuotaTruthPort` 和 `QuotaSourceFactory`。Service 按 `CallerIdentity + Capability + method + provider_instance_name` 读取 Provider quota 状态，并可叠加 system-config 中明确配置的预算及 WP-14 usage 聚合。`prepare_route` 只负责形成请求级、作用域绑定的 quota 观察快照；各候选由 PolicyEngine 分别求值。某个 Provider 的额度不支持、缺失、查询失败或状态未知时，该候选使用 `unknown`，不得阻断其它候选或整个请求；明确的 `exhausted` 和明确超出预算仍是候选级硬过滤。`quota.query` 由 Service 调用 factory 的异步查询入口，额度不可得时返回 `unknown`。

多币种契约补充：WP-09 已随 WP-01 的正式 `Money { amount, currency }` DTO 移除策略上限、quota snapshot 和 `quota.query` 中的 USD 专用金额。明确配置的单次 cost ceiling 与剩余 budget 只在币种标识完全一致时比较，不在 routing 内隐式换汇；非法的显式策略配置 fail closed，Provider 返回的非法或不可比较额度观察退化为 `unknown`，不阻断候选。
...
- [x] 按 lowering 固定的 execution mode 精确选择 buffered、streaming 或 native transport；
- [x] Provider streaming 中间态写入 TaskMgr event/task data；
- [x] AICC RPC 不新增独立 streaming transport；
- [x] 实现启动阶段 failover；
- [x] Provider native task 提交成功并取得 remote task ID 后固定 runtime/Adapter，不跨 Provider 重试；普通调用按 `aicc_router.md` 的错误分类执行同模型重试和候选切换；
...
- [x] 持久保存 canonical trace ID，并关联 TaskMgr 状态、终态事件和重启恢复；
- [x] 覆盖重启、并发、迟到 completion 和取消竞态。

完成标准：所有执行形态对调用方暴露一致的 task、event、error、usage 和 cancel 语义。

实现记录：Execution 小组在 `src/frame/aicc/src/execution/mod.rs` 实现统一的 `ExecutionEngine`，通过窄端口装配 TaskMgr、Provider runtime、持久执行状态和 usage completion。三种执行形态共享 task、event、error、usage 和终态语义；Provider stream 的 delta/progress 写入 TaskMgr data。WP-18 补齐的 production execution port 严格按 `ResolvedProviderCall.execution_mode` 选择 `send`、`send_streaming` 或 native submit，不再按 Adapter 支持集合优先猜测 Immediate；不支持的模式在 lowering 阶段以 `unsupported_execution_mode` 终止且不发起 Provider 请求。流式执行按顺序转发 Delta/Progress，只接受 Final 进入公共成功与 usage completion 路径；流内 Provider 错误、transport interruption 或无 Final EOF 均进入公共失败路径，取消沿用本地中止及 late-final 屏蔽。原生任务的 pinned binding 持久固定 runtime generation、Provider、Adapter、operation、remote task id 和无明文 secret 的 `NativeTaskResumeDescriptor`；descriptor 保存固定 base URL、credential reference/type/header/fingerprint、resolved parameters 与 Codec limits，恢复和取消禁止改用当前 Runtime 配置。`ProviderExecutionPort::start` 必须从本次固定 generation 返回 Native Task 及其 resume descriptor，poll/cancel 只接收持久 binding；旧 credential reference 已不可解析时任务以不可重试 `provider_error` 终结为 Failed。运行时 failover 仅允许发生在 Provider 接受请求前，提交成功或接受后失败均不跨 Provider 重试。幂等按 `tenant_id + method + idempotency_key` 定义至少 24 小时窗口，对去除 key 后的 canonical JSON 计算 SHA-256 fingerprint，并原样重放运行中、成功、失败或取消结果；cancel 仅在真实上游接受或本地执行成功中止并安装 late-final 屏蔽时返回成功。WP-01TS/WP-14/WP-16 对接契约已补齐：`ExecutionRequest` 和持久 `ExecutionRecord` 保存 canonical `trace_id`、`user_id`、原始 `request_model` 与基于 task id 确定生成的 `usage_event_id`；空 trace ID 在创建 task 前拒绝。`TaskSpec` 将同一 trace 写入不可变 TaskMgr input，状态 payload 显式携带该值，完成、失败和取消终态事件通过 `task_id` 回连该 input；native task 重启恢复始终使用 `ExecutionRecord` 中的原值，不生成新 trace。成功 `UsageCompletion` 直接提供 `event_id`、trace、用户/应用归属、capability、原始 request model、Provider 实例、`completed_at_ms` 和强类型 `Option<AiCost>` finance snapshot。`PinnedProviderTask` 还会从本次 `ResolvedProviderCall.pricing` 固定显式 currency 及 token（含 cache input）或 unit pricing；结算只使用该持久快照与最终 usage，不读取当前 catalog，也不使用隐含币种的 `estimated_cost_usd`。finance snapshot 仅接受本次 completion 的最终总成本并校验非负有限 amount 与非空 currency，路由估算或单价不进入该字段。usage 的 `provider_model` 查询维度明确写入带 Provider 实例的 selected exact model；裸 `provider_model_id` 只保留在 pinned binding 中供 Provider 调用和重启恢复。Execution 定向测试覆盖三种执行形态、完整状态映射、精确 mode 分派与 unsupported 拒绝、流事件归并、正常 Final、流内 Provider error、无 Final EOF interruption、stream cancel、trace 输入校验、TaskMgr 状态与终态关联、并发去重、body conflict、启动期 failover、固定 generation 与 resume descriptor/pricing/trace 持久恢复、credential 丢失时恢复/取消的终态行为、重启恢复后的完整 usage 归属、immediate/stream/native 最终费用、任务期间 catalog 价格更新隔离、缺失 usage、非法最终成本、租户隔离、Provider 不支持取消、取消/迟到完成竞态及输入归属校验；AICC 全量测试、all-target check、格式和 diff 检查通过。严格 clippy 的 Execution 改动未产生告警，当前全包 `-D warnings` 仅受并行模块既有告警阻断；完整 `buckyos-build.py --skip-web` 因环境缺少四个 `BUCKYOS_SDK_TOOL_*` 不可变输入而未执行。未新增依赖，真实 TaskMgr/storage/runtime 端口装配由 WP-16 完成。
...
实现记录：Storage/Observability 小组已在 `src/frame/aicc/src/storage/mod.rs` 实现基于平台 RDB instance 的 inventory LKGS、usage、route trace、audit 和 execution record 持久化，包含跨 SQLite/Postgres 的原子 inventory/metadata seq upsert、SHA-256 校验与坏行重建、completion usage 强制和双重去重、filter/group/bucket/cursor 查询、四级 trace ID 关联及诊断 retention；usage 持久事实显式记录 `user_id`、canonical `method`、`provider_instance_name` 和请求 `trace_id`，production usage writer、route trace、audit/event 通过同一 canonical `trace_id` 关联，execution record 持久保存该值以保证服务重启后不重新生成。对应过滤、聚合与 `[start_time_ms, end_time_ms)` 半开周期可直接供用户级和 method 级预算使用。usage 聚合返回按规范化币种稳定排序的 `finance_totals + finance_complete`：合法混合币种分别汇总且保持完整，缺失、非法或单币种求和溢出时保留其它合法小计并标记不完整；每次成功 completion 至少计一个 `consumed_request_units`。同一 RDB 上的 production `ExecutionStore` 以 tenant/method/idempotency key 原子 claim，持久保存完整执行记录和固定 Provider task binding，并用非终态条件 CAS 保证 complete/fail/cancel 竞争只有一个终态成功；`recoverable` 支持服务重启恢复，production `UsageCompletionPort` 将强类型 completion 成本幂等写入 usage。`src/frame/aicc/src/observability/mod.rs` 实现 latency/error/queue/health/refresh/snapshot generation metrics、统一递归脱敏和 secret/content 扫描。Storage/Observability 单元测试随 AICC 全量测试通过，all-target check 和格式检查通过；AICC 严格 clippy 在排除模块外既有 lint 后通过。ExecutionStore 与 UsageCompletionPort 的服务运行时装配由 WP-16 完成。
...
| Admission | quota/budget 候选级判定、privacy/trust/locked policy fail closed、quota unknown 保留候选 |
