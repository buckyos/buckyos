...
WP-08F 实现记录（已完成）：fal builtin 装配位于 `src/frame/aicc/src/provider/builtin/fal.rs`，Rust 仅保留稳定 `fal` Profile ID、catalog-only discovery 和无法声明化的 Queue 状态机行为。显示名称、默认 `https://queue.fal.run`、`fal-queue` Adapter、`Authorization: Key` credential 及 region/workspace/account 声明由 `driver_metadata/known-providers/fal.known-provider.json` 提供；14 个 image/audio/video API type 到 `queue.submit` 的 operation 映射由 `driver_metadata/providers/fal.provider.json` 提供，Rust 不再构造生产用 Known Provider、Provider Rules 或默认模型 inventory。根据 fal 官方 [About](https://fal.ai/about)、[Inference](https://fal.ai/docs/documentation/model-apis/inference) 和 [Queue](https://fal.ai/docs/documentation/model-apis/inference/queue) 文档，fal 是承载多来源生成媒体模型的推理平台，而非其目录内模型的统一原始发布厂商，因此不提供 `fal.model.json`；模型 ID、能力和原厂 metadata 由 WP-15 当前 catalog snapshot 配置提供。`src/frame/aicc/src/protocol/fal_queue.rs` 继续实现独立 `fal-queue / queue.submit` native-task Adapter，覆盖媒体 typed request lowering、submit/status/result/cancel、生命周期、官方错误、路径校验、资源归一化及 artifact 输出，并复用 WP-05 polling/deadline/backoff/cancel contract。两份 builtin catalog 已通过 `CatalogSnapshot` 解析，配置提供的模型可构建完整 inventory identity，且 fal catalog 明确不包含 Model Driver；fal 定向测试 19/19、AICC 全量测试 276/276、`cargo check -p aicc --all-targets`、stable Rust 格式、JSON 语法和 scoped diff 检查通过，未新增依赖。
