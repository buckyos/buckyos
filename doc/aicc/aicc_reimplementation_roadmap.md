...
WP-18 execution mode 契约补充：`route.resolve`、全部 typed inference request 和两个 Helper request 统一使用严格枚举 `execution_mode = immediate | stream`，缺失时默认为 `immediate`；公共错误码 `unsupported_execution_mode` 表示路由或 Adapter/operation 不支持所请求模式，禁止静默降级，且不向调用方暴露内部 `native_task`。
