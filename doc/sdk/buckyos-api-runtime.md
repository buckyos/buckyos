...
buckyos api runtime 是访问 buckyos system service 的基础运行时。当前正确的生命周期是:

- `init_buckyos_api_runtime()`: 构造一个未注册、未认证的 runtime 草稿，不写入全局 runtime
- `runtime.login()`: 验证或生成 session token，拉取 zone config，完成身份认证
- `set_buckyos_api_runtime(runtime)`: 将 runtime 注册为当前进程的全局 runtime；只有第一次调用可以成功，并在注册后启动后台 keep-alive

使用基本流程如下
```
runtime = init_buckyos_api_runtime() // 收集用于login的必要信息，创建runtime草稿
runtime.login()? // login结束后得到可验证的身份与后续系统调用所需的session token
set_buckyos_api_runtime(runtime)? // 注册成全局runtime
...
需要访问 buckyos 的服务，都应当在进程启动时按 `init -> login -> set` 顺序初始化 api-runtime。

## init 到 login 之间的语义

`init()` 到 `login()` 之间，runtime 可以持有:
...
- 还没有完成 session token 的确认
- 还没有把 runtime 注册成全局 runtime
- 其它模块此时调用 `get_buckyos_api_runtime()` 会失败

因此，这个阶段只能访问系统里那些不要求认证的只读能力，不能把 `user_id` 当成已经认证通过的最终身份。
...
标准环境有3个目录 (注意其选择是整体的，不会用优先级补足)，Owner 私钥只会存在客户端身份目录中：
