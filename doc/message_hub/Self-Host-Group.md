...
2. **可寻址投递**：可以作为 `MsgObject` 的 `from` 或 `to` target。任意一方向该 DID 收发消息时，从 DID Document 解析出 service endpoint 即可发起交互，不需要预先知道 DID 背后是个人、设备、agent 还是集合。
...
这个承诺让 BuckyOS 的 DID 体系保持精简，同时不扩张 CYFS Protocol 的身份边界：任何一段 MessageHub 代码、任何一个授权判断、任何一次消息投递，面对的永远只有“实体”和“实体的集合”两种主体。后面 self-host group 的所有设计——DID Document 作为权限根、双向成员证明、`from=author/to=[group]` 的消息语义、递归集合展开、协作作者/受益方表达、subgroup 不默认拥有独立 DID——都是在这个抽象承诺下的自然推论。
...
* 群消息的会话主体是 `to` 中 role 为 `Conversation` 的 `group_did` target。
* 群消息作者是 `from`。
...
MsgObject {
    from: author_did,
    to: [{ endpoint: group_did, role: "Conversation" }],
    kind: "chat",
    ...
}
```

也就是说，UI 或 agent 提交的 `from=author, to=[group]` 就是规范群消息对象。MessageCenter/Group 管理组件可以补充 target role、consumer 和 delivery hint，但不应把作者改写到额外 `source` 字段。
...
3. 外部平台回流的群消息必须能映射到 `group_did` 和作者 endpoint，不能只保留平台 chat id。
...
2. UI/Agent 提交规范 `MsgObject`：`from=author_did`，`to` 中包含 `group_did` 的 `Conversation` target。
...
pub struct ContentAttribution {
    ...
    pub author_did: DID,
    pub initiator_did: Option<DID>,
    pub attribution_context: Option<String>,
}
```

当 `author_did` 是 `group_did` 时，语义是“该内容由这个 DID 集合共同创作或共同负责”。如果某个具体成员或 agent 发起了发布，可以把它放在业务对象自己的 `initiator_did` / `publisher_did` 一类字段中；这属于内容业务语义，不复用 `MsgObject` 的群消息作者模型。
...
这些 API 可以复用已有 `dispatch()` / `post_send()`，但建议在接口层保留 group 语义，避免调用方手工拼错 `from/to/kind` 和 target role。
...
13. **作者与 group 分离**：`from=author_did` 表示真实作者，`to` 中的 `group_did` target 表示群时间线主体；在内容署名场景中，`author_did=group_did` 也应允许业务字段记录具体发布者。
...
8. MessageCenter 支持 `from=author, to=[group]` 的群消息语义。
...
1. `Message Center.md`：群消息 `dispatch/post_send` 的伪代码应与最终 `from/to` 规范一致。
2. `Contact Mgr.md`：`Contact.groups` 要明确为联系人集合，不等同于 messageable group。
3. MessageHub UI model：应直接使用 `from=author, to=[group]` 表达群消息，并在 target 上保留 Conversation role。
4. `ndn-lib::MsgObject`：确认新版 `MsgObject` 的 endpoint target、transport、content parts、stream 和 ext 字段已经落地；如果协议对象缺字段，必须先改协议和前后端镜像类型。
