...
- `MsgObject`：系统标准消息对象，定义在 `ndn-lib`。协议定义见 `doc/message_hub/MsgObject.md`。消息内容、作者、目标、thread、排序、传输通道和组件消费意图应优先由 `MsgObject` 核心字段表达；Message Tunnel 特异信息放入 `ext["buckyos.message_tunnel"]`。
...
本文建议新增或固化的对象，主要位于 `Message Tunnel` 抽象层，用于描述外部 IM 原始对象、平台能力、会话事件和转换契约。`Message Tunnel` 是 `MsgObject` 的一个应用场景，不应反向把平台字段塞进 `MsgObject` 核心字段；已经由 `from`、`to`、`via`、`thread`、`seq`、`content` 表达的信息，不应在扩展字段中重复出现。扩展字段只能保存平台差异信息，例如平台消息 ID、mention token、附件导入状态、平台 profile 补充、raw payload 或 checkpoint。
...
`MsgObject.content` 应承载标准化内容；平台特有结构放入 `ext["buckyos.message_tunnel"]`、part 级 `ext` 或附件引用。理论上 tunnel 需要识别以下内容类型：
...
4. Tunnel 将消息转换成 `MsgObject`。聊天内容进入 `MsgContent`；来源 actor 进入 `from`；会话、目标 DID 和消费组件进入 `to`；tunnel 通道进入 `via`；平台顺序或可排序的平台消息 ID 进入顶层 `seq`；平台消息 ID、无法放入 `thread` 的 thread/topic 细节、mention、平台文件 ID、checkpoint、无法标准化字段进入对应扩展字段。已经由核心字段表达的信息不进入扩展字段。
...
- 在 `ext["buckyos.message_tunnel"].mentions`、part 级 `ext` 或 `TunnelContentBlock::Mention` 中保留结构化 token。
- 能解析则映射到 DID 或联系人；不能解析则保留外部引用。
- 不把某个平台的 `@` 语义硬编码为所有平台通用语义。

### 8.3 入站非结构化数据

Tunnel 处理图片、文件、语音、视频、贴纸和未知二进制内容时，应按 `MsgObject` 的三种非结构化数据表达方式封装：

- 小型、体积受控且确实需要随消息内联的数据，使用 `MsgAttachment { body: MsgAttachmentBody::Inline { encoding, value }, ... }`，例如 `encoding="base64"`。
- 已在云端或外部系统可下载的数据，使用 `MsgAttachment { body: MsgAttachmentBody::Url(url), ... }`，URL 可以是 `http`、`https`、`ftp`、`cyfs` 等。
- 已导入本地 NDN / NamedObject 系统的数据，使用 `MsgAttachment { body: MsgAttachmentBody::ObjId(obj_id), ... }`。
- `MsgAttachment.mime`、`name`、`size`、`digest` 和 `meta` 只保存跨平台通用元信息；平台 file id、缩略图 token、上传/下载状态不放在这里。

平台自己的文件 ID、file key、Content-ID、缩略图 token、下载失败状态和可重试信息不替代上述正文表达方式；这些信息进入 `ext["buckyos.message_tunnel"].attachments`，并用 `part_id` 关联到 `content.parts`。
...
- `ext["buckyos.message_tunnel"]`：标准字段无法表达的 `reply_ref` 补充信息、`capability_overrides`、平台附件 ID、raw payload 等扩展；已经由 `from`、`to`、`via`、`thread`、`seq`、`content` 表达的信息不应重复写入扩展。
...
- 使用平台 sequence、message_id、timestamp 和 update_id 生成顶层 `MsgObject.seq`。排序范围由 `to[].endpoint`、`thread.id`、`stream.stream_id` 或平台分区语义共同确定；`seq` 可使用 `U64` 或 `Str`，同一范围内必须固定一种编码。
- 同一会话内乱序到达时，允许先写入，UI 和 Agent 通过 `MsgObject.seq` 重排；只有旧对象或损坏对象缺失 `seq` 时，才使用 `created_at_ms + MsgObjectId` 做展示级兜底。
...
- 未知消息类型进入 `Unknown` 内容块或 `ext["buckyos.message_tunnel"].raw` / `raw_ref`。
- 未知事件进入 `TunnelEventKind::Unknown`。
- 未知平台字段原样保留到 `ext["buckyos.message_tunnel"].raw` 或 `raw_ref`。
...
- Telegram group/channel/user 的 peer 类型若无法由 `from` / `to` 的 `External.kind` 表达，应保留在 `ext["buckyos.message_tunnel"].raw` / `raw_ref` 或 part 级 `ext`。
...
- Tunnel 应保留无法标准化的原始 JSON 到 `raw`，大型或完整原始 payload 使用 `raw_ref`。
...
- `ext["buckyos.message_tunnel"]`、part 级 `ext`、`raw` / `raw_ref` 字段只能追加，不应改变既有字段含义。
- 语义不可兼容的升级应增加 schema 或 protocol version，并提供迁移或降级策略。
- 遇到未知能力时，默认视为不支持，而不是默认允许。

### 16.3 数据安全

- Secret 不应出现在普通 `MsgObject.ext`、日志或 `RouteInfo.extra` 中。
