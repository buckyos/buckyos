...
* **场景D置了NAT的端口映射的情况**
- 映射了 80/443/2980 端口到OOD，此时可以看作DDNS模式
- 映射了任意端口（比如3980端口到OOD的2980端口），比较常见的是一个支持端口映射的网络里，需要暴露连个不同Zone的Gateway
  - 在rtcp看来，可以直接连接
  - 在http系列协议看来，需要通过SN中转（等同于场景C）
  该场景下，DNS查询的结果并不指向ZoneGate
