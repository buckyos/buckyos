...
构建dir的过程
临时数据库，path-type-objid
挨个计算path-file-objid
当出现path-dir时，观察其子文件的hash是否已经计算完成
计算完成，则使用数据库遍历进行dirobj的计算，计算完成后更新objid
    dirobj的实体会保存到named_mgr中

对于特别大的dirobj（这是极端情况），目前使用独立数据库保存（后续需要优化）
