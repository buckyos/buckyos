# Jarvis PDCA 行为提示词需求文档

PDCA是一个现实中的元提示词(无领域只是提示词)范式, 系统通过支持该组提示词，来确认OpenDAN Runtime有足够的能力来支持复杂的模式
实际中 ToC 的Jarvis使用简单的Plan->Execute-loop来完成工作，PDCA提示词组计划在企业级环境（面对普遍更大规模的任务）时使用
