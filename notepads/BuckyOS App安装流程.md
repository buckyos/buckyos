...
以下接口是 PKG 场景特有的上层逻辑，建立在 RepoService 之上。

| 接口 | 说明 | 产生的证明 |
|------|------|-----------|
| `app.create_instance(install_spec) → instance_id` | 基于 Install Spec 创建实例，写入调度器。**前提：content_id 在 RepoService 中 status=pinned。** | **install action**（`ActionObject(action=installed)`，通过 `repo.add_proof` 落盘） |
| `app.start_instance(instance_id)` | 启动实例，调度器拉起容器。 | 无 |
| `app.stop_instance(instance_id)` | 停止实例。 | 无 |
| `app.disable_instance(instance_id)` | 禁用实例（disabled 状态，不可自动恢复）。 | 无 |
| `app.remove_instance(instance_id)` | 删除实例，清理专属目录。用户数据等挂载目录不受影响。 | 无 |
...
用户通过任意渠道获得 pkg_meta
        ...
        │
        ▼
  repo.collect(pkg_meta, referral_action?)
  ...
  (若收费) 链上付费 → 获得 receipt
        │
        ▼
  downloader.download(content_id)
        │  协议层从网络拉取 PKG 实体
        │  传输完成时生成 download_action
        ▼
  repo.pin(content_id, download_action)
        │  哈希校验, status=pinned
  ...
  构造 Install Spec (UI 决策)
        │  权限、端口、数据挂载等
        ▼
  app.create_instance(install_spec)
        │  写入调度器目标状态
        │  ✎ proof 落盘：install action（记录"用户真实安装"）
        │      → 通过 repo.add_proof() 写入
        ▼
  app.start_instance(instance_id)
        │  容器启动，应用运行
        ▼
      应用运行
```
...
```
  ...
  repo.collect(new_pkg_meta)               ← 新版 meta 入库, status=collected
        ...
        ▼
  downloader.download(new_content_id)      ← 下载新版 PKG 实体
        │
        ▼
  repo.pin(new_content_id, download_action) ← 新版实体就绪
        │                                     ✎ download action 落盘
        ▼
  app.stop_instance(old_instance_id)       ← 停止旧实例
        │
        ▼
  构造新 Install Spec                       ← 继承旧配置 + 补充新字段
        │
        ▼
  app.create_instance(new_install_spec)    ← 创建新实例
        │                                    ✎ install action 落盘（新版本的安装证明）
        ▼
  app.start_instance(new_instance_id)      ← 新版本运行
```
...
```
  app.disable_instance(instance_id)        ← 停止容器，标记 disabled
```

**第二层：删除实例**（常规卸载）

```
  app.remove_instance(instance_id)         ← 清理实例专属目录
```

此时 PKG 仍在 RepoService 中（pinned 状态），可随时重新创建实例。
...
| **消费者** | collect + pin + create_instance | shared / download / installed action |
...
repo.store(pkg_path)                             
     │ content_id + meta 入库                     
     ▼                                           
repo.announce(content_id)                        
     │ meta 发布到 BNS                            
     ▼                                           
  社交传播 / BNS 可查                              
     │                                           
     │  A 分享给 B                                
     │  ✎ A 创建 referral_action ──────────►  B 获得 pkg_meta + referral_action
                                                    │
                                                    ▼
                                             repo.collect(meta, referral_action)
                                                    │ ✎ shared action 落盘
                                                    ▼
                                             (若收费) 链上付费 → receipt
                                                    │
                                             ...
                                             downloader.download(content_id)
     ┌──────────────────────────────────────────────┤
     │  生成 download_action                         │ 传输完成
     │  ✎ download action (serve端落盘)              ▼
     │                                       repo.pin(content_id, download_action)
     │                                              │ ✎ download action 落盘
     │                                              ▼
     │                                       构造 Install Spec (UI)
     │                                              │
     │                                              ▼
     │                                       app.create_instance(spec)
     │                                              │ ✎ install action 落盘
     │                                              ▼
     │                                       app.start_instance(id)
     │                                              │
     │                                              ▼
     │                                          应用运行
     │
