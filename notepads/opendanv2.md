...
- AgentMemory/Session/Worksapce 是不同的状态管理空间，让提示词中的 preload state更聚焦
- Behavior 区分了不同行为下的 process_rule,让提示词中的流程逻辑更聚焦
- 元工具 + Tool意图引擎，解决了围绕特定状态空间的扩展Agent动手能力的问题
- 与workflow引擎的集成，实现了与现有信息系统的边界清晰的集成（过渡期必须）
...
* Session 不存消息正文，只存"引用 MsgTunnle message_id 的链接"（MsgRecord.msg_id -> MsgObject)
...
> 实际产品结论：除非UI本身能做强独占（比如LLM的消息解析成模态选择框），否则基于规则的算法分发是不成立的
> 2次LLM虽然慢，但是稳。如果worksession并不多的话，其实损失不大
...
* `local_workspace` 若由 session 运行中创建，通常是 Agent 私有；
  若用户先手工创建 local workspace 再交给 Agent 工作，则该 local workspace 通常属于用户。

> 约束：**session 可以绑定 1 个 local_workspace 和 0..n 个 workspace**。
...
### 8.1 resolve_router （route→link 模型）

**目标**：将`新输入`投影（link）到合适的内部 Session，并施加安全与成本策略。在必要时做快速应答。
...
我们强烈的鼓励使用Action而不是Tool 

* **Call Tool（函数调用）**
  * LLM 产出 tool call
  * Runtime 执行 tool ccall
...
* Todo List 设计有单独文档：
  - 为什么todolist 有资格作为基础设施？为了通过get_next_ready_todo高效的驱动agent loop
  - 这是为了减少用LLM get_next_ready_todo引入的基础设施
