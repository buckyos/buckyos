...
- [ ] 先完成第 4 节的跨仓库协议前置项，再修改 Node Active；不能在 Node Active 内发明一个只对自己可见的临时 BNS 文档格式。
- [ ] 不新增第三方依赖。助记词和双密钥派生复用 `name-lib`；SN/BNS 调用复用新版 `buckyos-websdk` 和 `cyfs-gateway-api::SnClient`。
- [ ] 不修改 `src/rootfs/bin/node-active/` 中的构建产物；只改 `src/kernel/node_active/` 源码，最后由构建流程同步产物。
- [ ] 不在日志、错误、URL、localStorage 或 sessionStorage 中留下助记词、owner/device 私钥、SN token、密码或密码 hash。本地配置只允许沿用系统启动必需的 `admin_password_hash`；不得把钱包签名返回的 `pwd_hash`、SN 登录材料或其它重复副本落盘。
- [ ] 所有外部写操作必须有可重试、内容相关的幂等键；不能用固定 request id 重试不同内容。
- [ ] 激活的终点是“BNS/SN 权威状态可读 + 本地身份落盘成功”，不是“TX 已提交”。
- [ ] 保留用户工作区已有改动；当前 `product/node_active/Node_Active_现状说明_草稿.md` 和空的 `src/kernel/node_active/bns_client.ts` 都不是本 TODO 的生成产物，不得覆盖或回退。
...
- [ ] 将 `document` 从“只能 object”改为“JSON object 或非空 JWT string”；其它 JSON scalar/array 仍拒绝。
- [ ] `doc_type=owner` 仍只接受 JSON object，并保留现有 owner 身份字段不可更改的保护。
- [ ] `doc_type=zone` 接受 JWT string，按 UTF-8 原文写入 `DocumentRef::inline`；不得 `JSON.stringify` 成带引号的字符串。
- [ ] controller 增加通用 inline bytes/text 发布路径，继续执行 4KB 上限、expected version、name seq guard、controller policy、幂等和双重白名单。
- [ ] 计算幂等 payload hash 时包含 document 原文；同 request id 不同 JWT 必须明确失败。
- [ ] SN resolver 读取 canonical `name-lib::ZoneDocument` 时支持：
  - `devices[*].id`；
...
- [ ] 增加 E2E：SN proxy 发布 ZoneDocument JWT → bns-indexer 投影 → resolver 得到 `boot_jwt`、gateway device DID、mini JWT → device token 可被权威锚定。
- [ ] 更新 `SN-API.md`：object/JWT union、owner 限制、4KB、返回仍是 `status=submitted`、调用方必须等待投影。

### 4.2 buckyos-websdk：同步类型和客户端

主要入口：`buckyos-websdk` 的 `sn_client.ts`、`bns_tx_executor.ts`、`namelib`。

- [ ] `SnBnsPublishDocumentReq.document` 改为 `Record<string, unknown> | string`，`SnClient.publishDocument()` 原样发送 JWT string。
- [ ] 增加请求序列化测试，确认字符串没有被包成对象、没有二次 JSON 编码。
- [ ] 保持 `SnClientError.domainProofInfo()` 的结构化解析，Node Active 不解析错误 message 文本。
- [ ] 确认并测试以下导出可用：`sn.SnClient`、`bns.BnsClient`、`namelib.newOwnerDocument`、Zone/Device/Boot/Mini 构造和 JWT 编解码函数。
- [ ] 发布/锁定包含上述变更的明确 commit；Node Active 的 `pnpm-lock.yaml` 必须更新到该 commit，不能只依赖浮动 `#beta2.2`。
...
- [ ] 在 `name-lib` 公开安全的 12 词英文助记词生成函数，内部复用现有 `bip39` 和 CSPRNG。
- [ ] 返回值不得包含派生私钥；派生仍由现有 helper 完成。
- [ ] 加固定助记词测试向量，确保 owner 路径 `m/9777'/0'/0'` 与 EVM 路径 `m/44'/60'/0'/0/0` 和 BuckyOS App 一致。
- [ ] buckyos 更新 `name-lib` lock revision 后再实现 `/kapi/active` 的 material RPC。
...
- [ ] `WizardData` 增加 `owner_document`，owner DID/name/public key 都由转换函数从文档派生。
- [ ] 增加 `evm_address`、`domain_binding`、`signed_documents`；它们是明确状态，不用散落字段推断。
- [ ] 删除 `bns_evm_private_key`、`bns_evm`、`owner_private_key` 和旧手工 TXT 状态。
- [ ] `mnemonic_words` 只存在 Web 路径的 React 内存；组件卸载、成功或取消时主动清空。
- [ ] gateway/domain/owner/device key 任一输入变化时，使 `signed_documents` 失效；domain 变化同时使旧 `domain_binding` 失效。
...
- [ ] 建一个纯转换函数集中生成这些值并单测；active_server 不再对裸字符串调用模糊的 `DID::from_str(zone_name)` 后猜语义。
...
- [ ] 升级 WebSDK 后，从 `getCurrentWalletUser()` 读取 `owner_document`；没有该字段时视为钱包身份不完整，不回落到裸 public key 激活。
- [ ] 用 WebSDK/name-lib 解析 OwnerDocument，校验：
...
- [ ] `owner_user_name`、`owner_public_key` 不再从 bridge 的多个历史候选字段猜测；OwnerDocument 是唯一来源。
- [ ] 钱包路线仍跳过 Security，但初始化完成前不能渲染向导，错误需明确指出缺失/不一致字段。
...
- [ ] Security 内部改为 `生成身份材料 → 确认备份 → 注册中 → 完成` 子状态，不必新增顶层 Wizard step。
- [ ] 增加必填 email，trim 后做基本格式校验；最终规范化和唯一性以 SN 为准。
- [ ] 调用 `generate_web_owner_material`，展示 12 词助记词并要求用户完成随机词位确认；未确认不能调用注册。
- [ ] 只读展示 EVM address，说明它是 BNS name 的 `asset_owner`，可由同一助记词恢复。
- [ ] OwnerDocument 使用 `namelib.newOwnerDocument` 构造：
...
- [ ] 注册前做 OwnerDocument round-trip、敏感字段扫描、EVM address 一致性和 `< 4096 bytes` 校验。
- [ ] 使用新版 `sn.SnClient.register`，请求必须包含：
  - `name`；
  - `email`；
  - `pwd_hash`；
  - `active_code`；
...
- [ ] 注册使用长事务超时；只有 `need_bind_owner_key === false` 且响应包含 BNS 成功提交信息时才进入下一步。SN 纯本地降级结果视为失败。
- [ ] 保存 access/refresh token 到 Wizard 内存，提交激活前仍先 refresh。
- [ ] 删除“用户名已存在就自动 login”的路径。传统 Web 没有现有 owner 私钥/助记词时无法合法签署四份文档；用户名被占用就阻止注册，并提示用户改用 BuckyOS App/钱包中的已有身份。
- [ ] 删除旧文案中的“登录并绑定 owner key”。
...
- [ ] 默认 BNS 域名不调用 `domain.bind`，DomainStep 可直接继续。
- [ ] 自有域名输入通过本地格式校验后，必须取得新鲜 SN access token再调用 `SnClient.bindDomain(domain)`。
- [ ] Web 路径使用 Security 保存的 refresh/access token。
- [ ] 钱包路径若尚无 SN session，使用现有 `walletSignWithActiveDid` 对带 domain、owner DID、iat、随机 nonce 的 `buckyos.node_active.domain_bind` intent 做一次明确授权；使用返回的 `pwd_hash` 调 `auth.login`，签名结果不作为 SN proof，SN 仍只信外部 DNS。该 intent 仅用于让钱包弹出密码授权并取得当前既有 bridge 能力中的 pwd_hash。
- [ ] 捕获 `SnClientError` 且 `isSnError("domain_proof_failed")` 时，只用 `domainProofInfo()` 取得 challenge；不得解析 message 字符串。
- [ ] challenge UI 展示：record name、完整 PKX value、复制按钮、DNS 传播提示、重新验证按钮。
- [ ] 只有 `domain.bind` 成功响应中的 domain 与当前输入一致时写入 `DomainBindingState.verified` 并允许 Next。
- [ ] domain 被其它用户占用、owner PKX 无法解析、DoH 不可用等非 retryable 错误要区分展示，不得误报“TXT 尚未生效”。
- [ ] 删除 Domain/Review 中提前展示的 NS 委派提示，以及 Review 中 BOOT/DEV/PKX 手工生成和 `dnsReady` 逻辑。
- [ ] Success 在自有域名激活完成后展示最终 NS 记录；此提示不影响系统已激活状态，但需明确“完成 NS 后域名才会对外解析”。

## 8. P4：构造、签名并严格验证四份 Document

### 8.1 文档构造必须只有一个规范来源

- [ ] `prepare_active_documents` 在 Rust 使用当前 `name-lib` 类型构造文档，替换 `active_lib.ts::create_zone_boot_config()`、`create_device_mini_config()` 与 active_server 两边各造一份的现状。
- [ ] BootDocument 必须带明确 `id=zone_did`、`owner=owner_did` 和正确的 oods/sn/exp；不要继续依赖 decode 后补字段。
- [ ] DeviceDocument 的 `id` 使用 zone scoped DID，`owner`、`zone_did`、device key、net_id、rtcp、ddns_sn_url、support_container 与 topology 一致。
- [ ] DeviceMiniDocument 从同一 DeviceDocument 生成。
- [ ] `assemble_zone_document` 只接受已通过 owner key 验签的三个子 JWT；组装时复用原 prepare payload和 JWT，不重新生成时间字段。
- [ ] ZoneDocument 使用 owner key、zone DID、owner DID、`boot_jwt`、`devices`、`mini_device_jwts` 和 `oods/sn` 构造，`version_seq` 必须存在。

### 8.2 钱包签名

- [ ] 第一次 `walletSignWithActiveDid([boot, device, deviceMini])` 必须返回三个非空 JWT 和 `pwd_hash`。
- [ ] 把三个 JWT 送给 `assemble_zone_document`，再调用 `walletSignWithActiveDid([zoneDocument])`。
- [ ] 第二次返回必须有一个非空 ZoneDocument JWT；若返回 pwd_hash，必须与第一次相同。
- [ ] 用户取消任一次签名时停留在 Review，不 publish、不登记设备、不落盘。
- [ ] 禁止在 console 输出四份 JWT、pwd_hash 或完整 WizardData。

### 8.3 Web 签名

- [ ] `sign_web_active_documents` 接收助记词，只在请求处理期间派生 owner key；签完立即 drop，不存入 server struct、文件或环境变量。
- [ ] 派生公钥和 EVM address 必须与 OwnerDocument 完全一致，否则拒绝。
- [ ] 使用与钱包路径相同的 prepare payload和 assemble 逻辑，不能保留另一套文档形状。

### 8.4 commit 前服务端验证

`commit_active` 不能信任浏览器已经校验。至少验证：

- [ ] OwnerDocument 的 DID/name/default key/EVM wallet 格式和内部一致性。
- [ ] 四份 JWT 全部能用 OwnerDocument default key 验签。
- [ ] JWT claims 分别能反序列化为正确的 `name-lib` 类型。
- [ ] ZoneDocument 的 `boot_jwt` 与提交的 Boot JWT 逐字节相同。
- [ ] ZoneDocument 内的 DeviceDocument 与提交的 DeviceDocument payload语义相同，mini JWT 与提交值逐字节相同。
- [ ] Boot/Zone 的 zone DID、owner DID、oods、sn；Device 的 id/name/zone/owner/key/net_id/rtcp；Mini 的 name/key/rtcp 全部互相一致。
- [ ] device private key 派生的公钥与 DeviceDocument default key 一致。
- [ ] 当前请求的 gateway/domain mapping 与签名内容一致，防止签完后篡改 UI 参数。
- [ ] 四份文档均未过期，`iat/exp/version_seq` 满足 name-lib 规则。
- [ ] ZoneDocument JWT UTF-8 长度 `< bns_client::MAX_INLINE_DOCUMENT`。典型单 OOD 当前约 3KB，但必须用测试守住 4KB 上限。
...
- [ ] owner request id 使用 `node-active:owner:<owner-name>:<sha256(canonical-owner-json)>`。
- [ ] zone request id 使用 `node-active:zone:<owner-name>:<sha256(zone-jwt)>`。
- [ ] 不用固定 request id 覆盖变更后的 gateway/domain 内容。
- [ ] SN 返回 `status=submitted` 只表示已投递，不等于成功；通过 `bns.BnsClient`/Rust `BnsIndexerClient` 轮询 document version 和原文。
- [ ] 轮询使用有限 deadline、指数或有上限的退避、明确进度状态；禁止固定 sleep 后假定成功。
- [ ] owner publish 投影后再发 zone publish，避免两个写操作使用相同旧 `name_seq`。
- [ ] projection 超时返回可重试错误，保留同一 request id；不写本地激活完成标志。
...
- [ ] `node_identity.json` 仍保存 owner DID + pinned owner public key作为本地 Root Trust，不允许在线 OwnerDocument key rotation 静默替换它。
- [ ] 本地保存 `device_doc.jwt`、`device_mini_doc.jwt`，并新增 canonical `zone_document_jwt` 持久化/缓存入口；Boot JWT仍可从 ZoneDocument 取得。
- [ ] DID cache 写入 `DidDocType::Zone` 的 ZoneDocument JWT，并保留需要的 Boot cache，均使用 `UpdateSource::Authority`。
- [ ] `start_config.json` 保存完整公开 OwnerDocument、ZoneDocument JWT、三个子 JWT、本地系统设置及系统启动必需的 `admin_password_hash`；清除 mnemonic、全部 private key、access/refresh token、钱包签名返回的 `pwd_hash` 和其它重复临时副本。
- [ ] 日志脱敏名单增加 `mnemonic`、`mnemonic_words`、`zone_document_jwt`、`owner_document_jwt`（若未来出现），避免整个签名文档落日志。
- [ ] `StartConfigSummary` 增加 typed OwnerDocument；`SystemConfigBuilder::add_user_doc()` 使用本次获取/注册的完整 OwnerDocument，不再按 `user_name + public_key` 合成一份丢失 display/wallet/bound zone 的最小文档。
- [ ] profile 从真实 OwnerDocument 的 name/display_name/avatar/meta 构造。
- [ ] 自有域名时，本地使用已成功发布且含 zone binding 的 OwnerDocument；默认同名 Zone 保留隐式绑定语义。
- [ ] 所有文件先写临时文件并原子 rename，远端已成功但本地写一半时不能留下“可启动但身份不完整”的目录。
...
- [ ] 12 词助记词生成可被自身 parse。
- [ ] 与 BuckyOS App 相同固定助记词派生出相同 owner public JWK、owner PEM 和 EVM address。
...
- [ ] 典型单 OOD ZoneDocument JWT 小于 4KB；构造超限内容时在 publish 前失败。
- [ ] `start_config.json`/日志快照不含 mnemonic、private key、SN token 或钱包签名返回的 `pwd_hash`；`admin_password_hash` 只在现有必需的配置位置出现，且不进入日志或额外副本。
...
- [ ] `active_lib.ts::create_zone_boot_config()` / `create_device_mini_config()` 自造 schema。
- [ ] `generate_zone_txt_records` 及 Review 的 BOOT/DEV/PKX 手工 DNS UI。
- [ ] `register_sn_user(name, pwd_hash, active_code)` 缺 email/asset_owner/owner_config 的旧签名。
- [ ] username taken → `auth.login` 的传统 Web 自动登录分支。
- [ ] `bns_evm_private_key`、`BNS_PRIVATE_KEY`、`BnsEvmControllerClient` 在 Node Active 激活路径中的使用。
- [ ] `active_server.rs::bns_zone_document()`、`bns_boot_document()`、`bns_device_mini_document()` 三份临时 BNS JSON。
- [ ] 把 `sn_url.is_some()` 同时当成控制面、relay、publish、device online 判据的逻辑。
- [ ] 任何旧 bind zone RPC 或旧 domain verify RPC 调用。

## 13. 完成定义

只有同时满足以下条件，CodeAgent 才能标记完成：

- [ ] 两条路径都以完整 OwnerDocument 为身份输入，任何裸 key fallback 已删除。
- [ ] Web 快速注册包含必填 email、同助记词 owner/EVM 双派生和可恢复备份确认。
- [ ] Boot、Device、DeviceMini、Zone 四份 JWT 均真实签名并在服务端严格互验。
- [ ] BNS `zone` document 保存的是签名 ZoneDocument JWT 原文，不是无签名聚合 JSON。
- [ ] 默认同名 Zone 只 publish zone；自有域名完成 PKX proof 并发布 owner binding + zone。
- [ ] TXT proof 在 NS 委派之前；NS 只在激活成功后提示。
- [ ] SN 返回 submitted 后等待 BNS 最终投影，未确认前不落盘、不退出。
- [ ] 真实 OwnerDocument 进入本地 system-config，owner root key 仍由 `node_identity` pin 住。
- [ ] 不再需要用户 EVM 私钥或 `bns_evm_private_key` 完成 Node Active。
- [ ] cyfs-gateway、WebSDK、name-lib、buckyos 的相关单测和构建均通过。
- [ ] 交付说明列出跨仓库 commit、主要入口文件、实际验证、未覆盖的钱包/DNS 外部环境风险。
