...
| Executor Reference | 节点的 executor 字段。实际定义形如 `service::aicc.helper.llm_chat` / `http::x.y` / `appservice::z` / `operator::json.pick` / `func::<objid>`；语义链接形如 `/agent/mia` / `/skill/fs-scanner` / `/tool/...`，运行前由 registry 展开。 | DSL §3.2，Service §6.1 |
