...
配置覆盖 AI Provider、模型名、提示词、工具开关、可用 Bash 工具列表和 Runs 目录等执行设置。父目录可以维护多个命名提示词组，子目录选择一个组，再定制其中的 section；命令行也可通过 `--select <name>` 选择组。组可以附带默认任务要求，减少每次手工输入。标准模式的每个 system section 都有行号，按行号升序拼接；系统为部分行号提供固定别名，用户可通过指定其它行号插入自己的 section。需要完全控制业务提示词时，也可以配置整段自定义内容。执行循环依赖的协议提示词由系统管理，不属于可替换的业务内容。
...
以下是一份完整的 `project/.llm_context` 示例，覆盖模型访问、Runs 目录、执行限制、工具、提示词组与 system section。这里采用 YAML 表达拟定的配置结构，字段名与正式解析规则由后续 CLI/SDK 协议落实，不表示当前命令已经支持该格式。示例假设模型访问环境已配置名为 `team-ai` 的 Provider 和逻辑模型 `code-default`，凭据沿用该 Provider 的配置；使用时应替换为实际可用的名称，`rg` 和 `git` 也须已安装并位于当前 Bash 的 `PATH` 中。

```yaml

# 使用buckyos时，会使用taskmgr等buckyos服务
provider: 
  # 默认是buckyos 还可以选openai
  type: buckyos
  # 可以手工配置session_token来解决buckyos的身份问题
  session_token : xxxx 
...
# 支持 function_call(默认) 和 behavior 两种
loop_model: behavior
# 运行过程中的输出模式，按信息的丰富程度支持 debug,info,warn,result  4个级别
run_logs: info
# 如果 xllm > result.out ,result_format会影响结果
# 默认是raw (llm返回是啥就是啥)， 用result.xxx 说明llm最终返回了一个json/xml的结构化数据，只取其中的一个字段
result_format: result.report


prompt:
  mode: standard
  # 配置后，可以cd到特定目录直接 xllm 执行review 任务
  select: review
  groups:
    # 支持吧prompt写在单独的文件里
    ...
    review:
      ...
      sections:
        ...
        contexts:
          text: |
            本项目是 Rust 服务，错误通过 Result 返回。
            这段文字补充项目背景；当前时间、时区、操作系统和工作目录由系统填入。
        "25":
        ...
        rules:
          text: |
            ...
            工具使用范围以系统提供的本次可用工具说明为准。
        cmd_manual:
          text: |
            搜索代码时优先使用 search_code；查看变更时使用 inspect_changes。
            命令、用途和参数以系统根据 bash_tools 生成的 exec 手册为准。
        output_format:
          text: |
            用中文回答，按严重程度列出问题。
            每项包含文件位置、依据和建议；没有发现问题时直接说明。
      # tools是广义提示词的一部分，因此提示里也可以覆盖配置
      tools:
        # 整体开关，关闭后一次xllm调用就是一次推理
        enabled: true
        # 使用behavior模式的时候，自动把tools转换成actions(tools为空)
        tools2actions: true
        tools:
          # 标准的read_file,write_file,exec
          - groupname : bash
          # 通过mcp协议地址添加一组tools
          - mcp : http://xxxx
          # 不属于bash组的内置tool sendmsg 
          - name : sendmsg 
        # behavior模式支持同时配置tools和actions,但一般不会这么做
        # actions:
        #  - groupname : bash
    docs:
      default_user: |
      ...
      sections:
        ...
        output_format:
          text: 用 Markdown 表格列出文档位置、问题和建议。
  sections:
    "50":
...
示例中，`prompt.groups` 定义可选组，`prompt.select` 选择默认组；`default_user` 是该组的默认任务要求，进入 user 输入。组内 `sections` 提供该组的 system 内容，`prompt.sections` 是对所有选中组生效的目录级覆盖或新增内容。每个 section 的键使用固定别名或带引号的数字行号，`text` 是用户文本，`name` 是自定义 section 的可选名称；同一集合不要同时声明同一行号及其别名。

默认选中 `review` 时，system 依次包含 **10 role → 20 contexts/env → 25 project_conventions → 30 rules → 40 cmd_manual → 50 delivery_checklist → 100 output_format**，最后追加系统管理的 `runtime_protocol`。时间与环境、实际可用工具、exec 命令手册按 F11 自动填入，无需配置模板变量或手写协议。切换到 `docs` 后不再带入 `review` 的 25 行及其它组内文本，目录级 50 行仍生效。

`tools.enabled: true` 显式启用工具；`bash_tools` 提供扩展命令及其手册信息，不能代替工具开关。三个执行限制分别表示单次模型输出 token 上限、工具轮数上限和本次命令超时秒数。`runs_dir` 指定记录位置；若改为相对路径，则以声明它的 `.llm_context` 所在目录解析。

在 `project/src/.llm_context` 中只写差异即可。例如，覆盖执行规则和项目约定，用数字 100 覆盖组内的 `output_format`，并清空继承的 50 行：
...
prompt:
  ...
  sections:
    ...
    "50":
      text: ""
    "100":
      text: 用中文列出问题、代码位置与依据，最后给出建议的验证步骤。
```

该子目录继承父目录的 Provider、模型、工具和组定义；最终 section 行号为 **10、20、25、30、40、100**，50 行连同标题一起省略，100 行不会与 `output_format` 重复输出。假设从 `project` 的父目录开始，可按以下方式使用：
...
前两次任务使用显式选中的组；`src` 中的 `xllm --file ./lib.rs` 提交材料并使用目录默认的 `review` 组，无需问题参数，因为继承了 `review.default_user`。最后一条命令用显式问题替换默认任务，并用 `--no-tools` 覆盖文件中的工具开关，仅处理提交的文件。无参数且无管道输入的 `xllm` 仍按 F01 显示用法。

若需要完整自定义业务提示词，可将上面完整配置中的整个 `prompt` 块替换为以下内容，其余模型、工具和限制配置继续保留；此时由命令行显式给出本次问题，不再使用组默认任务：
...
`mode: custom` 不同时声明 `select` 或 `sections`；运行时协议、实际工具声明和显式输出约束仍由系统提供。
...
系统在新 Run 构造提示词时为 `contexts` / `env` 提供本次时间和实际运行环境，用户可补充项目背景。`rules` 中的可用工具说明、`cmd_manual` 中的 exec 命令手册由系统根据生效的工具开关与 Bash 工具列表提供，用户可补充使用规则、命令示例和注意事项，无需重复抄写系统已提供的说明。关闭工具时，`rules` 明确说明没有可调用工具，不提供可执行的命令手册；未启用的命令不能被描述为可用。
...
2. **计算每个 section 的最终文本**：将固定别名归一到行号，依次取“系统默认值 → 选中组的值 → 合并后的目录 section 覆盖值”，后层声明的同一行号整段替换前层的可配置文本，新行号作为独立 section 加入。未声明则继承，显式空文本则清空，不把多个配置来源的文本叠加。切换组时重新计算，不保留上一个组的文本。随后按 F11.1 填入本次环境、可用工具和 exec 命令说明。
...
- 配置来源、选择的组、最终模式、各 section 的行号、固定别名或自定义名称、最终内容与来源、运行时协议内容及版本、拼接后的提示词和实际任务要求应随任务保存，确保查看时能核对组装结果。resume 直接使用保存的提示词、输入与本次 Run 的进度，不重新执行目录配置合并或 section 组装，不因组定义修改而更换任务，也不重新填入时间或环境信息；当前执行器无法处理保存的协议版本时明确报错，不静默换用新协议。
...
首版交付 F01–F11 中的用户体验，包括：位置参数直接提问、`--select` 运行常用提示词组、文字和图片输入、文本材料、xllm 之间的管道串联、`.llm_context` 向上查找与合并、Provider/模型配置、section 行号与固定别名、按行号插入自定义 section、提示词组、完整自定义提示词、工具开关与 Bash 工具列表、可配置 Runs 目录、干净 context 的独立任务与按编号恢复、最近任务列表、状态与结果查询、结构化结果、执行限制和中断处理。可恢复错误只使本次命令失败，原任务可反复 resume；终态任务只读展示结果，不得重新执行。跨任务只通过用户显式选择的输入传递材料，不提供持续 session。
...
| A41 | 环境、工具与命令说明 | 新 Run 的 contexts/env 包含本次时间与实际环境，rules 说明实际可用工具，cmd_manual 说明可用 exec 命令及调用方法；用户覆盖或清空文本不会移除系统生成的必要说明；关闭工具后说明无可调用工具，不将命令手册作为可执行能力提供；提示词不能启用工具或改变执行权限 |
...
- 后续 SDK/CLI 技术设计需要落实位置参数与 `--select` 的解析、组默认任务要求、`.llm_context` 文件格式与字段、F11 规定的 section 行号、固定别名、自定义插入、冲突校验与具体拼接格式、环境信息和工具/命令说明的填入、运行时协议与解析器的成套装配及最终答案提取规则、配置来源追踪、管道输入输出与执行时序、输出字段、可恢复错误分类与退出码、Runs 目录格式和恢复保证；遇到产品行为变化时，应同步修订本 PRD。
