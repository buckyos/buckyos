...
系统默认按下面的顺序进行备份。新设计里，不再按旧的 `dfs://system / dfs://usr / fs://cache` 分区表达，而是按 `/config/*` 与 `cyfs://` 逻辑目录表达。（UI 上依然可以允许某个 app 或 service 声明更高优先级）

- `/config/boot/config` 与 `/config/system/*`：系统启动与关键控制面配置。优先级 50
- 优先级为 20 的应用/服务持久数据
- `cyfs://$zone_id/home/$userid/`：用户个人数据。优先级 17
- `cyfs://$zone_id/home/$userid/shared/`：用户级共享数据。优先级 15
- `cyfs://$zone_id/srv/library/`：Zone 级共享资料。优先级 13
- `cyfs://$zone_id/srv/publish/`：Zone 级发布数据。优先级 10
- `cyfs://$zone_id/home/$userid/.local/share/$appid/`：应用永久数据。应用可声明自己的备份优先级组，我们保留 5 个组：-20、-10、0、10、20，默认 0，最高 20
- `cyfs://$zone_id/srv/$service_name/`：服务持久数据。服务也可声明相同的备份优先级组，默认 0
- `cyfs://$zone_id/cache/$service_name/`、`/tmp/buckyos/$appid`：缓存/临时数据，默认优先级为 -25，通常不要求完整备份
...
5. 一个原始扇区可以产生多个备份扇区（多个副本），并在系统关键配置区中保存 `checkpoint->原始扇区->备份扇区` 的映射关系。
6. 向Backup Target发备份扇区后，删除本地的备份扇区。
7. 在系统持久存储区保存原始扇区的`部分存根数据`后，可以删除原始扇区（一般会保存至少 1 个 checkpoint 的原始扇区用来做增量备份的 base 计算）
8. 在保存了系统关键配置备份的 Backup Target 上进行一些元数据设置，方便查询“最新的系统关键配置”对应的备份扇区 id
...
该设备在翻新后加入 Zone 后，Zone 内拥有两个有效的 OOD，`cyfs://` 数据层的有效空间也会增加。
对高级用户来说，也可以手工替换损坏的硬盘，此时该硬盘的数据会自动恢复。

不停机恢复依靠的是 BuckyOS 的内部机制实现（SystemConfig 和 `cyfs://` 数据层内置的数据修复机制），不需要访问 Backup Target 服务器。
...
2. 向 Backup Target 查询包含了系统关键配置的最新备份扇区 id（手工恢复可以跳过第 1、2 步的查询）
3. 向 Backup Target 请求这些最新备份扇区
4. 基于私钥解密备份扇区的AES密钥，解密备份扇区的数据，得到原始扇区
5. 通过原始扇区重建系统关键配置
6. 启动系统（SystemConfig 服务和 `cyfs://` 数据层启动），并让系统进入恢复模式
7. 在恢复模式下读取 SystemConfig 中的记录，得到重建所有数据的扇区列表
8. 按备份扇区的优先级，依次请求 Backup Target 的备份扇区，并在 `cyfs://` 逻辑数据层上重建数据
9. 恢复完成，系统进入正常模式
