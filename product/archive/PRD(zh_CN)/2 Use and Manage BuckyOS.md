...
在详细备份与恢复的相关机制之前，我们有必要先详细说明 BuckyOS 当前的数据管理机制。

- 所有数据最终都会落到本地文件系统上，但对应用和服务暴露的是统一的逻辑路径模型，而不是宿主机路径本身。
- 结构化配置保存在 `/config/*`（SystemConfig，只读映射到应用/服务进程；写入必须通过 SDK API）。
- 非结构化文件数据保存在 `cyfs://` 命名空间；单机版/桌面版会直接映射到 `$buckyos_root/data/`。
- 对应用来说，看到的是固定的 Linux 风格沙箱目录；不会再暴露旧的 `dfs://` / `fs://cache` 分区模型。

- 站在 Node Local OS 的角度（通常是 Linux），本地目录需求可概括为：
  - `/opt/buckyos_backup/`：发布时的二进制和必要初始配置，只读，用于恢复。
  - `/opt/buckyos/bin/`：系统二进制目录，升级时写入。
  - `/opt/buckyos/etc/`：宿主机配置。
  - `$buckyos_root/data/`：`cyfs://$this_zone_id/` 的本地映射，承载用户数据、服务数据与共享数据。
  - `$buckyos_root/storage/`：内核基础设施持久化存储（如 chunks、named_store），卸载默认不删除。
  - `/tmp/buckyos/`：本机临时目录，保存应用和服务的临时数据，可随时清理。

使用 docker 启动时，依然可以整体挂载一个 buckyos 根目录：
...
- 当前设计下，主要逻辑目录划分为：
  - `/config/*`：SystemConfig 的只读视图，例如 `/config/boot/config`、`/config/users/$userid/settings`
  - `cyfs://$zone_id/home/$userid/`：用户个人数据
  - `cyfs://$zone_id/home/$userid/.local/share/$appid/`：应用永久数据
  - `cyfs://$zone_id/home/$userid/shared/`：用户级共享数据
  - `cyfs://$zone_id/srv/library/`：Zone 级共享资料库
  - `cyfs://$zone_id/srv/publish/`：Zone 级发布数据
  - `cyfs://$zone_id/var/$service_name/`：服务运行数据
  - `cyfs://$zone_id/cache/$service_name/`：服务缓存数据
  - `cyfs://$zone_id/srv/$service_name/`：服务持久数据

站在 app 开发者的角度，在容器里默认应看到：

- `/tmp/`：临时数据；实际落到 `/tmp/buckyos/$appid`
- `/config/*`：系统与用户配置的只读视图
- `/home/$userid/`：用户 home，通常只读或不可访问，具体取决于授权
- `/home/$userid/.local/share/$appid/`：默认可读写的应用永久数据区
- `/srv/library/`、`/srv/publish/`：在获得授权后访问的共享/发布目录

站在用户的角度，主要看到的是自己的 home 与共享目录，而不是旧的 `dfs://` 根目录：

- `/home/$userid/`
- `/home/$userid/shared/`
- `/srv/library/`

如果是系统管理员，也应通过逻辑路径和 SDK/控制面板访问数据，而不是假设存在一个直接可见的 `dfs://` 根挂载。
