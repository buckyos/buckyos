...
2. **A/B 职责分离**：A 管技术事实，B 管运营 overlay。B 不允许修改 A-only 字段。
3. **发布前可解释**：发布前必须展示 diff、影响范围、风险字段、污染字段 warning 和生成的测试建议。
4. **高密度专业工具**：A 服务面向工程师，允许 JSON/schema、规则、批量选择、模式匹配等专业操作。
5. **低代码运营工具**：B 服务面向运营管理员，主要通过表单、开关、批量调整和预设策略完成配置。
...
- 通常基于 A 服务已经发布的 provider/model 做调整。
...
1. A/B WebUI 都提供浏览模式和编辑模式，默认进入浏览模式。
2. 进入编辑模式时创建 `edit_session`，顶部固定显示 base revision、操作者、编辑状态和未发布变更数量。
3. A 服务支持 provider、model、model rules、model nicks、patterns/defaults/variants/version_rules、logical directory、api_type、capability 的浏览、搜索、详情和编辑。
4. A 服务支持以已有 provider/model 为模板创建新对象。
5. A 服务支持导入 YAML/Markdown 更新计划，导入后进入待提交状态，不直接发布。
6. B 服务支持配置 A 服务 URL、手动同步、查看 A revision/B revision、查看 stale 状态。
7. B 服务首版运营字段固定为 disabled、pricing override、routing weight、recommendation level、display priority，并支持 provider/model/pattern/block 的运营 overlay 编辑和批量调整。
...
| 一级模块 | 页面 | 说明 |
|---|---|---|
| Dashboard | 概览 | A/B revision、A 同步状态、stale 提示、发布状态、warning 数。 |
| Tech Source | A 服务源 | A 服务 URL、最近同步时间、手动刷新、同步结果。 |
| Providers | Provider 运营 | 浏览 A 字段，只编辑禁用、推荐级别、展示优先级、运营备注。 |
| Models | Model 运营 | 浏览 A 技术字段，只编辑价格覆盖、routing weight、推荐级别、禁用、灰度。 |
...
A 服务导入强调 schema、selector 和 key 字段检查。B 服务导入强调运营字段白名单，不展示复杂 JSON patch 作为主路径。
...
- 专属规则：provider scope 下的 patterns/defaults/variants/version_rules。
...
- 调整优先级时预览最终命中结果。
- 删除规则前显示将新增或移除的模型样例。
- 黑名单命中的模型在 A 服务发布中保留但设置 `exclude=true`。
...
- A revision / B revision 对照。
- A 服务同步状态：正常、同步中、失败、stale。
- 最近一次成功同步时间。
- 当前 overlay 数量。
...
- 手动刷新失败时保留上一版可用 A 缓存，并在顶部显示 stale。
- 如果 A revision 变化，B 服务应提示当前 overlay 将基于新 A 数据重新预览。
...
- 面向 A 服务管理员可使用技术术语，如 provider、model、schema、revision、capability。
- 面向 B 服务管理员优先使用业务词，如「禁用」「推荐级别」「运营价格」「同步状态」。

英文：

- 保持字段名和 API 术语稳定，例如 Provider、Model、Revision、Overlay、Published JSON。
- B 服务避免使用 Patch、Schema、Resolver 等非必要技术词作为主路径文案。
...
| `metadata.tech_source.update` | 否 | 是 | 修改 A 服务 URL。 |
...
- A/B 字段所有权必须由服务端校验，不能只依赖前端禁用控件。
- B overlay 中出现 A-only 字段时，发布阶段丢弃污染字段并记录 warning。
...
B 服务无法拉取 A 服务最新数据但有上一版缓存时：

- Dashboard 显示 stale。
- Provider/Model 列表仍可浏览上一版 A 数据。
- 发布确认页必须显示 stale 风险。
- Stale 状态下允许 B 服务发布，但发布确认页必须要求管理员确认使用的 A revision。
- Change log 必须记录发布时使用的 A revision 和 stale 状态。

---

## 12. 关键流程

### 12.1 A 服务新增 OpenRouter 聚合 Provider

1. 工程师进入 A 服务 Providers。
2. 点击「新增聚合 Provider」。
...
5. 进入发布预览。
6. 发布后 B revision 更新，客户端下一次拉取时移除该 provider。

### 12.5 B 服务配置 A 服务源

1. 运营管理员进入 Tech Source。
2. 输入 A 服务 URL。
3. 点击测试连接。
4. 测试成功后点击手动刷新。
5. 系统显示 A revision 和同步摘要。
...
1. 可以配置 A 服务 URL、测试连接、手动刷新。
2. Dashboard 展示 A revision 和 B revision。
3. A 字段在 B 服务中只读，无法通过 UI 解锁。
4. 可以禁用 provider/model，并在发布预览中看到最终不下发。
5. 可以批量调整运营价格、routing weight 和推荐级别。
6. overlay 中出现 A-only 字段时，发布预览显示污染字段 warning。
7. 可以导出最终发布 JSON。
...
- B 服务首版运营字段最小集合固定为 disabled、pricing override、routing weight、recommendation level、display priority。
- 移动端只允许浏览和紧急禁用，不允许发布。
