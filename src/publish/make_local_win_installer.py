...
def generate_nsis_script(
    *,
    ...
    lines.append("  ${EndIf}")
    lines.append("")
    lines.append("  nsExec::ExecToStack 'powershell.exe -NoProfile -ExecutionPolicy Bypass -Command \"if (Test-Path ''\\\\.\\pipe\\docker_engine'') { exit 0 } else { exit 1 }\"'")
    lines.append("  Pop $0")
    lines.append('  ${If} $0 != 0')
    lines.append('    StrCpy $DockerCheckCode "3"')
    lines.append("    Return")
    lines.append("  ${EndIf}")
    lines.append("FunctionEnd")
    lines.append("")
