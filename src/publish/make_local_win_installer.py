...
def generate_nsis_script(
    *,
    ...
    lines.append('  ${EndIf}')
    lines.append(
        '  nsExec::ExecToLog \'powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$$root = $\\"$0$\\"; $$script = Join-Path $$root \'\'bin\\\\stop.py\'\'; if (-not (Test-Path $$script)) { Write-Host \'\'stop.py not found, fallback to taskkill\'\'; taskkill /F /IM node_daemon.exe; exit 0 }; $$py = Get-Command py.exe, python.exe, python3.exe -ErrorAction Stop | Select-Object -First 1; & $$py.Source $$script"\''
    )
    lines.append("FunctionEnd")
    lines.append("")
    ...
    for comp in components:
        ...
        if comp.system_service:
            ...
            lines.append(f'    Call RunStopScript')
