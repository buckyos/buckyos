...
dest_dir = os.path.join(temp_dir, "deb_build/deb/opt/buckyos")
shutil.copytree(src_dir, dest_dir, dirs_exist_ok=True)
print(f"copy rootfs to {dest_dir}")
dest_dir = os.path.join(temp_dir, "deb_build/deb/")
print(f"chmod -R 755 {dest_dir}")
os.system("chmod -R 755 " + dest_dir)
clean_dir = os.path.join(temp_dir, "deb_build/deb/opt/buckyos/etc")
...
dest_dir = os.path.join(temp_dir, "deb_build")
build_cmd = f"cd {dest_dir} && dpkg-deb --build deb"
os.system(build_cmd)

print(f"build deb success at {dest_dir}")
copy_cmd = f"cp {dest_dir}/deb.deb {build_dir}/buckyos.deb"
os.system(copy_cmd)
