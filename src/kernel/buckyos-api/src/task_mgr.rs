...
/// URL-safe opaque Task ID owned by TaskMgr. Callers must not parse the format
/// or infer creation time or tree relationships. Zone-trusted delegated
/// controllers may reserve a canonical id when a frozen business plan must
/// bind the Task identity before creation; ordinary callers must not mint ids.
...
/// runner binding are one durable transaction. `task_id`, when present,
/// reserves the canonical `t-<32 lowercase hex>` identity already frozen into
/// the delegated business plan.
#[derive(Debug, Clone, Serialize, Deserialize)]
