//! Workflow 编译态 / 运行态共享的中间表示类型。
//!
//! - `RefPath` / `ValueTemplate`：DSL 引用 (`${node.output...}`) 的解析结果，
//!   `submit_definition` / `get_definition` / `get_run_graph` 这些 RPC 都会
//!   把它们以 JSON 形式返回，所以是 wire schema 的一部分。
//! - `ExecutorRef` / `Expr`：编译产物（Expr Tree）的节点表示，外部工具想做
//!   静态分析或可视化 DAG 也需要这套类型。
//! - `RetryPolicy`：从 `RetryGuard` 规整后的运行时策略。
//!
//! workflow crate 内部的 compiler / orchestrator / adapter 都通过这里复用，
//! 不再持有副本。

use crate::workflow_dsl::{GuardConfig, OutputMode, RetryFallback, RetryGuard, StepType};
...
///   的形式（例如 `service::aicc.helper.llm_chat`、`http::file-classifier.classify`、
