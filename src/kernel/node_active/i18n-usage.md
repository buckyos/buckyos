...
本项目支持自动检测系统语言，并提供九种语言的国际化支持。所有UI组件和文本都已支持i18n。

## 功能特性

### 1. 自动语言检测
- 激活页支持 URL 查询参数 `lang`，例如 `http://192.168.1.10:3182/index.html?lang=zh`。BuckyOS App 的局域网扫描和远程设备入口会传入 App 当前语言。
- 初始化优先级：有效的 `lang` 参数 → 激活页本地缓存 `node_active.language` → 浏览器/系统语言 → 英语。未知或空的 `lang` 参数会被忽略。
- `lang` 支持 `zh`、`zh-TW`、`en`、`es`、`fr`、`de`、`ko`、`ja`、`ru` 及已有地区别名（如 `zh-CN`）；原有查询参数和 URL 片段保持保留。
- `lang` 只决定本次页面初始化语言，用户仍可通过页面语言菜单切换并保存选择；带有 `lang` 的 URL 刷新后仍优先采用该参数。
- 自动检测浏览器/系统的语言设置
- 支持的语言：简体中文 (zh)、繁体中文 (zh-TW)、英语 (en)、西班牙语 (es)、法语 (fr)、德语 (de)、韩语 (ko)、日语 (ja)、俄语 (ru)
- 如果检测到的语言不支持，默认使用英语
