...
// Some variables to configure log level via environment variables
const SYSTEM_CONSOLE_LOG_LEVEL_KEY: &str = "SYSTEM_CONSOLE_LOG_LEVEL";
const SYSTEM_FILE_LOG_LEVEL_KEY: &str = "SYSTEM_FILE_LOG_LEVEL";
...
pub struct LogModuleConfig {
    ...
    // If set to Off, no console log will be printed
    pub console: LogLevel,

    // If false, no file log will be printed
    pub file: bool,

    // If None, no file log will be printed
    pub file_name: Option<String>,

    // Single log file max size, in bytes
...
impl LogModuleConfig {
    ...
    fn load_console_level_from_env() -> Option<LogLevel> {
        match std::env::var(SYSTEM_CONSOLE_LOG_LEVEL_KEY) {
            Ok(val) => match LogLevel::from_str(&val) {
                Ok(level) => Some(level),
                Err(e) => {
    ...
    fn load_file_level_from_env() -> Option<LogLevel> {
        match std::env::var(SYSTEM_FILE_LOG_LEVEL_KEY) {
            Ok(val) => match LogLevel::from_str(&val) {
                Ok(level) => Some(level),
                Err(e) => {
                    println!("parse env log level error! {}, {}", val, e);
    ...
    pub fn load(&mut self, node: &toml::value::Table) {
        ...
        for (k, v) in node {
            match k.as_str() {
                ...
                "file_max_size" => match TomlHelper::decode_to_int::<u64>(v) {
                    Ok(v) => self.file_max_size = v,
                    Err(e) => println!("decode log.toml file_max_size field error! {}", e),
                },

                "file_max_count" => match TomlHelper::decode_to_int::<u32>(v) {
        ...
        if let Some(level) = Self::load_console_level_from_env() {
            println!(
                "load module console level from env: mod={}, level={}, old={}",
                self.name, level, self.console
            );
            self.console = level;
        }
        ...
        if let Some(level) = Self::load_file_level_from_env() {
            println!(
                "load module file level from env: mod={}, level={}, old={}",
...
impl LogConfig {
    pub fn new(log_dir: PathBuf) -> Self {
        if let Err(e) = std::fs::create_dir_all(&log_dir) {
            println!("create log dir error! {}, {}", log_dir.display(), e);
        }
    ...
    pub fn load(&mut self, config_node: &toml::Value) -> Result<(), String> {
        let node = config_node.as_table().ok_or_else(|| {
            let msg = format!("invalid log config format! content={}", config_node,);
            error!("{}", msg);
    ...
    pub fn disable_module_log(&mut self, name: &str, level: &LogLevel) {
        ...
        // If the module is already configured, do not override it
    ...
    // Disable some async-std related log
