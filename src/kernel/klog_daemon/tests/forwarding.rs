...
#[tokio::test]
async fn test_three_node_strong_read_query_via_follower_auto_forwards() -> Result<(), String> {
    if !can_bind_localhost() {
        eprintln!("skip follower-forward strong-read query test: localhost bind is not available");
        return Ok(());
    }
    ...
    let port2 = ports[1];
    let port3 = ports[2];
    let cluster_name = format!("klog_query_forward_{}_{}_{}", port1, port2, port3);
    let mut nodes = spawn_three_voter_cluster(&cluster_name, port1, port2, port3).await?;
    ...
    let result = async {
        ...
        let appended = append_log(
            &client,
            leader_rpc_port,
            "strong-read-forwarded-from-follower",
            Some(700),
            Some(leader_id),
        )
        .await?;
        ...
        if queried.items.len() != 1 {
            return Err(format!(
                "unexpected strong-read query result len: expected=1, got={}",
                queried.items.len()
            ));
        }

        let item = &queried.items[0];
        if item.id != appended.id || item.message != "strong-read-forwarded-from-follower" {
            return Err(format!(
                "unexpected strong-read queried item: id={}, message={}",
                item.id, item.message
            ));
        }
