...
fn make_record(service: &str, time: u64, content: &str) -> SystemLogRecord {
    SystemLogRecord {
        ...
        time,
        file: Some("pipeline_transient_append_failure_test.rs".to_string()),
        line: Some(1),
...
#[tokio::test]
async fn test_pipeline_transient_server_append_failure_then_success() {
    let root = new_temp_root("transient_append_failure");
    let storage_dir = root.join("server_storage");
    let bind_addr = match allocate_bind_addr() {
        Ok(addr) => addr,
        Err(e) => {
            eprintln!(
                "skip pipeline_transient_append_failure test due socket restriction: {}",
                e
            );
    ...
    let node = "node-transient";
    let service = "svc_transient";

    let records = vec![
        make_record(service, 1722001900001, "transient-1"),
        make_record(service, 1722001900002, "transient-2"),
        make_record(service, 1722001900003, "transient-3"),
        make_record(service, 1722001900004, "transient-4"),
    ];
    ...
    let fail_times = 2usize;
    let wrapped: LogStorageRef = Arc::new(Box::new(FailThenOkStorage {
        inner: real_storage.clone(),
        remaining_failures: AtomicUsize::new(fail_times),
        append_calls: append_calls.clone(),
    }));

    let server = LogHttpServer::new(wrapped);
    ...
    assert!(append_calls.load(Ordering::SeqCst) >= fail_times + 1);
