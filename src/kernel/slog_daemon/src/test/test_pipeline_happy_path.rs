...
fn make_record(service: &str, time: u64, content: &str) -> SystemLogRecord {
    SystemLogRecord {
        ...
        time,
        file: Some("pipeline_test.rs".to_string()),
        line: Some(1),
...
async fn test_pipeline_happy_path_service_to_daemon_to_server() {
    ...
    let bind_addr = match allocate_bind_addr() {
        ...
        Err(e) => {
            eprintln!(
                "skip pipeline_happy_path test due socket restriction: {}",
                e
            );
    ...
    let node = "node-e2e";
    let service = "svc_pipeline";

    let input_records = vec![
        make_record(service, 1722000000001, "pipeline-happy-1"),
        make_record(service, 1722000000002, "pipeline-happy-2"),
        make_record(service, 1722000000003, "pipeline-happy-3"),
    ];
    ...
    // Give server a short warm-up time before uploader starts requests.
    tokio::time::sleep(Duration::from_millis(100)).await;
