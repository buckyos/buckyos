...
fn make_record(service: &str, time: u64, content: &str) -> SystemLogRecord {
    SystemLogRecord {
        ...
        time,
        file: Some("pipeline_corrupt_sidecar_test.rs".to_string()),
        line: Some(1),
        content: content.to_string(),
    }
}

fn prepare_mixed_service_logs(
    log_root: &Path,
    service: &str,
    valid_records: &[SystemLogRecord],
    invalid_lines: &[&str],
    ...
    for (i, record) in valid_records.iter().enumerate() {
        ...
        if i < invalid_lines.len() {
            content.push_str(invalid_lines[i]);
            if !invalid_lines[i].ends_with('\n') {
                content.push('\n');
            }
        }
    }
    for line in invalid_lines.iter().skip(valid_records.len()) {
...
#[tokio::test]
async fn test_pipeline_mixed_valid_invalid_lines_sidecar_and_upload() {
    let root = new_temp_root("mixed_valid_invalid");
    let storage_dir = root.join("server_storage");
    let bind_addr = match allocate_bind_addr() {
        Ok(addr) => addr,
        Err(e) => {
            eprintln!(
                "skip pipeline_mixed_valid_invalid test due socket restriction: {}",
                e
            );
    ...
    let node = "node-corrupt";
    let service = "svc_corrupt";

    let valid_records = vec![
        make_record(service, 1722000500001, "valid-1"),
        make_record(service, 1722000500002, "valid-2"),
        make_record(service, 1722000500003, "valid-3"),
    ];
    let invalid_lines = vec![
        "this is an invalid raw line",
        "2024-01-01_00:00:00.000_+00:00 [info] [svc_corrupt] <only_file> malformed",
    ];
    let service_dir =
        prepare_mixed_service_logs(&root, service, &valid_records, &invalid_lines).unwrap();
    ...
    let mut expected_contents: Vec<String> = valid_records
        .iter()
    ...
    let corrupt_text =
        wait_for_corrupt_log_lines(&service_dir, invalid_lines.len(), Duration::from_secs(8))
            .await
            .unwrap();
    assert!(corrupt_text.contains("this is an invalid raw line"));
    assert!(corrupt_text.contains("<only_file> malformed"));
