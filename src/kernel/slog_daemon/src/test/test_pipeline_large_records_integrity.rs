...
fn make_large_record(service: &str, time: u64, idx: usize, size: usize) -> SystemLogRecord {
    let prefix = format!("large-record-{}-", idx);
    let mut content = prefix.clone();
    if size > prefix.len() {
        content.push_str(&"x".repeat(size - prefix.len()));
    }
    SystemLogRecord {
        level: LogLevel::Info,
        target: service.to_string(),
        time,
        file: Some("pipeline_large_records_test.rs".to_string()),
        line: Some(1),
        content,
    }
}
...
#[tokio::test]
async fn test_pipeline_large_records_integrity() {
    let root = new_temp_root("large_records_integrity");
    let storage_dir = root.join("server_storage");
    let bind_addr = match allocate_bind_addr() {
        Ok(addr) => addr,
        Err(e) => {
            eprintln!(
                "skip pipeline_large_records_integrity test due socket restriction: {}",
                e
            );
    ...
    let node = "node-large";
    let service = "svc_large";

    let record_count = 12usize;
    let record_size = 64 * 1024; // 64 KiB per record.
    let records: Vec<SystemLogRecord> = (0..record_count)
        .map(|i| make_large_record(service, 1722002000000 + i as u64, i, record_size))
        .collect();
    ...
    assert!(uploaded.iter().all(|s| s.len() >= record_size));
