...
fn make_record(service: &str, time: u64, content: &str) -> SystemLogRecord {
    SystemLogRecord {
        ...
        time,
        file: Some("pipeline_upload_timeout_test.rs".to_string()),
        line: Some(1),
...
async fn run_timeout_server_once(addr: String, accepted: Arc<AtomicBool>) -> Result<(), String> {
    let listener = tokio::net::TcpListener::bind(&addr)
        .await
        .map_err(|e| format!("failed to bind timeout server {}: {}", addr, e))?;
    let (mut stream, _) = listener
        .accept()
        .await
        .map_err(|e| format!("timeout server accept failed: {}", e))?;

    accepted.store(true, Ordering::SeqCst);
    let mut buf = [0u8; 1024];
    let _ = tokio::time::timeout(Duration::from_secs(1), stream.read(&mut buf)).await;

    // Keep connection open long enough so uploader request times out.
...
async fn wait_for_timeout_server_accept(
    accepted: &AtomicBool,
    timeout: Duration,
) -> Result<(), String> {
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            return Err("timeout waiting timeout-server accept".to_string());
        }
...
#[tokio::test]
async fn test_pipeline_upload_timeout_then_recover() {
    let root = new_temp_root("upload_timeout_recovery");
    let storage_dir = root.join("server_storage");
    let bind_addr = match allocate_bind_addr() {
        Ok(addr) => addr,
        Err(e) => {
            eprintln!(
                "skip pipeline_upload_timeout_recovery test due socket restriction: {}",
                e
            );
    ...
    let node = "node-timeout";
    let service = "svc_timeout";
    let records = vec![
        make_record(service, 1722001800001, "timeout-1"),
        make_record(service, 1722001800002, "timeout-2"),
        make_record(service, 1722001800003, "timeout-3"),
    ];
    prepare_service_logs(&root, service, &records).unwrap();

    let accepted = Arc::new(AtomicBool::new(false));
    let timeout_server = tokio::spawn(run_timeout_server_once(bind_addr.clone(), accepted.clone()));
    ...
    wait_for_timeout_server_accept(accepted.as_ref(), Duration::from_secs(8))
        .await
        .unwrap();
    timeout_server.await.unwrap().unwrap();
