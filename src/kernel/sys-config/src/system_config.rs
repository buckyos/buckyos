...
impl SystemConfig {
    pub async fn new(etcd_servers: &Vec<String>,node_device_name:&String) -> Result<Self, Box<dyn std::error::Error>> {
        if etcd_servers.len() == 0 {
            //TODO: enter local db model:
            return Err("etcd_servers is empty".into());
        }
        
        //TODO: Prefer to use the ETCD Server of this machine
        let endpoints: Vec<Endpoint> = etcd_servers.iter().map(|s| Endpoint::new(s)).collect();
        let config = ClientConfig::new(endpoints);
        //TODO: get auth info from os.env
