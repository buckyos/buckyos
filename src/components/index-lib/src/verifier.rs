...
impl Verifier {
    pub async fn verify(author: &str, chunk_id: &str, sign_base64: &str) -> IndexResult<()> {
        let (auth_key, remote_did_id) = resolve_ed25519_auth_key(author).await.map_err(|e| {
            IndexError::VerifyError(format!(
        ...
        if sign_bytes.len() != 64 {
            return Err(IndexError::VerifyError(format!(
        ...
        let signature = Signature::from_bytes(&sign_bytes.try_into().map_err(|e| {
            IndexError::VerifyError(format!("conversion to fixed-size array failed: {:?}", e))
        })?);

        public_key
            .verify(chunk_id.as_bytes(), &signature)
            .map_err(|e| IndexError::VerifyError(format!("verify failed, error: {:?}", e)))?;
