...
use index_lib::{IndexStore, PackageMeta, PkgVerifier};
use log::*;
use ndn_lib::ChunkId;
...
impl PackageRepository {
    ...
    pub fn add_package(
        ...
        self.zone_index_store
            ...
            .map_err(|e| {
                ...
                PkgError::DbError(e.to_string())
            })?;
        Ok(())
    }

    // version_desc version or chunk_id
    pub fn remove_package(&self, name: &str, version_desc: &str) -> PkgResult<()> {
    ...
    ) -> PkgResult<(Option<PackageMeta>, bool)> {
    ...
    pub async fn prepare_pkg(
        ...
        name: &str,
        version_desc: &str,
    ) -> PkgResult<(PackageMeta, Vec<PackageMeta>)> {
        // TODO: 先同步的解决所有依赖
    ...
    pub async fn pull_pkg(&self, meta_info: &PackageMeta) -> PkgResult<()> {
        unimplemented!("pull_pkg")
    }
}
