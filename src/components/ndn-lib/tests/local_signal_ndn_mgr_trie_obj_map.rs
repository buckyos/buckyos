...
//     obj_map.save().await.expect("flush trie-obj-map failed");

//     assert_eq!(
//         obj_map.len().await.expect("get trie-obj-map len failed"),
//         5,
//         "trie-obj-map total size check failed"
//     );

//     let verifier = TrieObjectMapProofVerifier::new(HashMethod::Sha256);
//     for (name, (chunk_id, chunk_data)) in chunks.iter() {
//         let obj_id = obj_map
//             .get_object(*name)
//             .await
...
//                 &obj_map
//                     .get_obj_id()
//                     .expect("trie-obj-map id should be calc finish"),
//                 &proof,
//             )
...
//     let is_ok = verifier
//         .verify(
//             &obj_map.get_obj_id().expect("trie-obj-map id should calc finish"),
//             &fake_obj_proof,
//         )
...
//         .calc_obj_id()
//         .await
//         .expect("calc trie-obj-map id failed for got trie-obj-map");
//     assert_eq!(got_obj_map_id, obj_map_id, "trie-obj-map id check failed");
//     assert_eq!(
//         got_obj_map.len().await.expect("get trie-obj-map len failed"),
//         chunks.len(),
//         "trie-obj-map total size check failed"
//     );
...
//     let mut append_obj_map = obj_map.clone(false).await.expect("clone trie-obj-map failed");
...
//         .get_obj_id()
//         .expect("append trie-obj-map id should be calc finish");
