    ...
    if let Some(expect_value) = &expect_value {
        match expect_value {
            Some(expect_value) => match &got_ret {
                ...
                Err(err) => assert!(false, "get file object {:?} failed, {:?}", file_obj_id, err),
            },
            None => match &got_ret {
                Ok(got_obj) => {
                    assert!(got_obj.is_null(), "should no object found")
...
async fn ndn_local_2_mgr_o_link_innerpath_file_ok() {
...

    {
        ...
        let (mut reader, resp_headers) = target_ndn_client
...

    {
        ...
        loop {
            let read_len = {
                let remaining = chunk_data.len() as u64 - read_pos;
                if remaining <= 1 {
                    remaining
                } else {
                    let mut rng = rand::rng();
                    rng.random_range(1u64..remaining)
                }
            };
            ...
            assert_eq!(
                ...
                "content-length in http-header should equal with file_obj.size, expect range: {:?}, chunk length: {}", read_pos..end_pos, chunk_data.len()
            );
            assert_eq!(
                resp_headers.obj_id,
                Some(chunk_id.to_obj_id()),
            ...
            //TODO: NDN get just use range.start, not range.end
            // assert_eq!(
            //     len as u64, read_len,
            //     "length of data in http-body should equal with content-length"
            // );
...
async fn ndn_local_2_mgr_o_link_innerpath_file_verify_failed() {
...

    {
        ...
        match ret {
            ...
            Err(err) => {
                error!("download_chunk_to_local error: {:?}", err);
                assert!(true, "download_chunk_to_local should error");
            }
        }
...
async fn ndn_local_2_mgr_r_link_innerpath_file_ok() {
...

    {
        ...
        loop {
            ...
            assert_eq!(
                content_len, file_obj.size,
                "content-length in http-header should equal with file_obj.size"
            );
            assert_eq!(
                resp_headers.obj_id,
                Some(chunk_id.to_obj_id()),
...
async fn ndn_local_2_mgr_r_link_innerpath_file_verify_failed() {
...

    {
        ...
        match ret {
            Ok(_) => assert!(false, "chunk-content should verify error"),
            Err(err) => {
                assert!(true, "Unexpected error type, {:?}", err)
            }
        }
