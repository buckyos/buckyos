...
/// 从 Value 创建 JWK
fn get_jwk(value: &Value) -> jsonwebtoken::jwk::Jwk {
...
impl<'a> UserEnvScope<'a> {
    ...
    pub fn create_owner_config(&self) {
        ...
        let owner_jwk = get_jwk(&self.key_pair.public_key_jwk);
    ...
    /// 创建 Zone 配置
    pub fn create_zone_config(&self, zone_did: &DID, sn_host: Option<String>) -> String {
        ...
        if let Ok(owner_x) = get_x_from_jwk(&get_jwk(&self.key_pair.public_key_jwk)) {
            println!("# {} TXT Record: PKX=0:{};", zone_host_name, owner_x);
        }
    ...
    pub fn create_node_config(
        ...
        // 2. 创建设备配置和 JWT
        let device_jwk = get_jwk(&device_key_pair.public_key_jwk);
        ...
        let identity_config = NodeIdentityConfig {
            ...
            owner_public_key: get_jwk(&self.key_pair.public_key_jwk),
        ...
        if device_name.starts_with("ood") {
            let start_config = json!({
                ...
                "public_key": get_jwk(&self.key_pair.public_key_jwk),
...
pub async fn create_sn_config(builder: &DevEnvBuilder) {
    ...
    let owner_keys = TestKeys::sn_owner();
    let device_keys = TestKeys::sn_device();

    // 写入设备密钥
    write_file(&sn_dir.join("device_key.pem"), device_keys.private_key_pem);
...
pub async fn create_test_env_configs() {
    ...
    // Bob Zone
    let _bob_zone_jwt = bob_scope.create_zone_config(
        &DID::new("bns", "bob"),
        Some("sn.buckyos.io".to_string()),
    );

    // Bob OOD1
    let bob_ood1_keys = TestKeys::bob_ood1();
    let _bob_ood1_did = DID::new("dev", "iSMKakFEGzGAxLTlaB5TkqZ6d4wurObr-BpaQleoE2M");
    bob_scope.create_node_config("ood1", &bob_ood1_keys, &DID::new("bns", "bob"), false);
...
pub async fn test_all_dev_env_configs() {
    ...
    let zone_boot_jwt = scope.create_zone_config(&zone_did, None);

    // 创建节点身份配置
    let node_identity_config = NodeIdentityConfig {
        zone_did: zone_did.clone(),
        owner_public_key: get_jwk(&owner_keys.public_key_jwk),
    ...
    let start_config = json!({
        ...
        "public_key": get_jwk(&owner_keys.public_key_jwk),
        "user_name": "devtest",
        "zone_name": zone_did.to_host_name(),
        "BUCKYOS_ROOT": "/opt/buckyos"
    });
    ...
    if let Ok(owner_x) = get_x_from_jwk(&get_jwk(&owner_keys.public_key_jwk)) {
        println!("# test.buckyos.io TXT Record: PKX=0:{};", owner_x);
    }
    if let Ok(ood_x) = get_x_from_jwk(&get_jwk(&device_keys.public_key_jwk)) {
