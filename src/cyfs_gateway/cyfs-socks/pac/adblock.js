...
extend(ActiveFilter, Filter, {
    ...
    isActiveOnDomain: function(docDomain, sitekey)
    {
        ...
        while (true)
        {
            ...
            docDomain = docDomain.substring(nextDot + 1);
        }
        return this.domains[""];
    },
...
function RegExpFilter(text, regexpSource, contentType, matchCase, domains, thirdParty, sitekeys)
{
    ...
    {
        var regexp = new RegExp(regexpSource.substring(1, regexpSource.length - 1), this.matchCase ? "" : "i");
        this.regexp = regexp;
    }
    else
    {
...
RegExpFilter.fromText = function(text)
{
    ...
    if (match)
    {
        ...
        for (var _loopIndex6 = 0; _loopIndex6 < options.length; ++_loopIndex6)
        {
            ...
            else if (option.charAt(0) == "~" && option.substring(1) in RegExpFilter.typeMap)
            {
                if (contentType == null)
                {
                    contentType = RegExpFilter.prototype.contentType;
                }
                contentType &= ~RegExpFilter.typeMap[option.substring(1)];
            }
...
Matcher.prototype = {
    ...
    findKeyword: function(filter)
    {
        ...
        {
            var candidate = candidates[i].substring(1);
