...
use windows::Win32::NetworkManagement::NetManagement::{
    NERR_Success, NetApiBufferFree, NetUserAdd, NetUserChangePassword, NetUserDel, NetUserEnum,
    NetUserGetInfo, FILTER_NORMAL_ACCOUNT, MAX_PREFERRED_LENGTH, UF_SCRIPT, USER_INFO_1,
    USER_PRIV_USER,
};
...
use windows::Win32::Security::{
    InitializeSecurityDescriptor, IsValidSecurityDescriptor, LookupAccountNameW,
    SetSecurityDescriptorDacl, ACL, NO_INHERITANCE, PSECURITY_DESCRIPTOR, PSID, SID_NAME_USE,
};
use windows::Win32::Storage::FileSystem::{
    NetShareAdd, NetShareDel, NetShareEnum, NetShareGetInfo, NetShareSetInfo, ACCESS_READ,
    SHARE_INFO_502, STYPE_DISKTREE,
};
use windows::Win32::System::SystemServices::SECURITY_DESCRIPTOR_REVISION;

fn add_user(user_name: &str, passwd: &str) -> SmbResult<()> {
    let mut name = OsString::from(format!("{}\0", user_name))
        .encode_wide()
        .collect::<Vec<_>>();
    let mut passwd = OsString::from(format!("{}\0", passwd))
        .encode_wide()
        .collect::<Vec<_>>();
    let mut comment = OsString::from("buckyos user\0")
        .encode_wide()
        .collect::<Vec<_>>();
    let ui = USER_INFO_1 {
    ...
    unsafe {
        let ret = NetUserAdd(
            PWSTR::null(),
            1,
            (&ui as *const USER_INFO_1) as *const u8,
            Some(&mut param_err),
        );
        if ret != 0 {
            Err(smb_err!(
                SmbErrorCode::Failed,
                "NetUserAdd failed: {}, param_err {}",
                ret,
                param_err
            ))
        } else {
...
fn delete_if_buckyos_user(user_name: &str) -> SmbResult<()> {
    ...
    unsafe {
        ...
        if ret == NERR_Success {
            ...
            let user = p.read();
            let comment = OsString::from_wide(user.usri1_comment.as_wide())
                .to_string_lossy()
                .to_string();
...
fn change_user_password(user_name: &str, oldpassword: &str, newpassword: &str) -> SmbResult<()> {
    let mut name = OsString::from(format!("{}\0", user_name))
        .encode_wide()
        .collect::<Vec<_>>();
    let mut oldpasswd = OsString::from(format!("{}\0", oldpassword))
        .encode_wide()
        .collect::<Vec<_>>();
    let mut newpasswd = OsString::from(format!("{}\0", newpassword))
        .encode_wide()
        .collect::<Vec<_>>();
    unsafe {
        let ret = NetUserChangePassword(
            PWSTR::null(),
            PWSTR::from_raw(name.as_mut_ptr()),
...
fn add_share(
    ...
    let mut netname = OsString::from(format!("{}\0", share_name))
        .encode_wide()
        .collect::<Vec<_>>();
    let mut path = OsString::from(format!("{}\0", share_path))
        .encode_wide()
        .collect::<Vec<_>>();
    let mut remark = OsString::from(format!("{}\0", share_remark))
        .encode_wide()
        .collect::<Vec<_>>();
    unsafe {
        let mut acl_list = Vec::new();
        for user in allow_users.iter() {
            let mut name = OsString::from(format!("{}\0", user))
                .encode_wide()
                .collect::<Vec<_>>();
        ...
        InitializeSecurityDescriptor(sd, SECURITY_DESCRIPTOR_REVISION).map_err(into_smb_err!(
            SmbErrorCode::Failed,
            "InitializeSecurityDescriptor failed"
        ))?;
        SetSecurityDescriptorDacl(sd, true, Some(acl), false).map_err(into_smb_err!(
            SmbErrorCode::Failed,
...
fn set_share_allow_users(share_name: &str, allow_users: Vec<String>) -> SmbResult<()> {
    unsafe {
        let mut netname = OsString::from(format!("{}\0", share_name))
            ...
            .collect::<Vec<_>>();
        let mut buf: *mut u8 = std::ptr::null_mut();
        let ret = NetShareGetInfo(
            PWSTR::null(),
            PWSTR::from_raw(netname.as_mut_ptr()),
            502,
            &mut buf,
        );
        if ret != 0 {
        ...
        let sd = if IsValidSecurityDescriptor(share.shi502_security_descriptor).0 != 0 {
            ...
            let mut buf = [0u8; 20];
            let sd = PSECURITY_DESCRIPTOR(buf.as_mut_ptr() as *mut c_void);
            InitializeSecurityDescriptor(sd, SECURITY_DESCRIPTOR_REVISION).map_err(
                into_smb_err!(SmbErrorCode::Failed, "InitializeSecurityDescriptor failed"),
            )?;
            sd
        };
        SetSecurityDescriptorDacl(sd.clone(), true, Some(acl), false).map_err(into_smb_err!(
            SmbErrorCode::Failed,
            "SetSecurityDescriptorDacl failed"
        ))?;
        LocalFree(HLOCAL(acl as *mut c_void));
    ...
    unsafe {
        loop {
            ...
            if ret == ERROR_SUCCESS.0 || ret == ERROR_MORE_DATA.0 {
                ...
                for _ in 0..tr {
                    let share = p.read();
                    let shi502_path = OsString::from_wide(share.shi502_path.as_wide())
                        .to_string_lossy()
                        .to_string();
                    let shi502_netname = OsString::from_wide(share.shi502_netname.as_wide())
                        .to_string_lossy()
                        .to_string();
...
fn delete_share(share_name: &str) -> SmbResult<()> {
    let mut name = OsString::from(format!("{}\0", share_name))
        .encode_wide()
        .collect::<Vec<_>>();
    unsafe {
    ...
    unsafe {
        loop {
            ...
            if ret == ERROR_SUCCESS.0 || ret == ERROR_MORE_DATA.0 {
                ...
                for _ in 0..tr {
                    let share = p.read();
                    let _shi502_netname = OsString::from_wide(share.shi502_netname.as_wide())
                        .to_string_lossy()
                        .to_string();
                    let shi502_remark = OsString::from_wide(share.shi502_remark.as_wide())
                        .to_string_lossy()
                        .to_string();
...
pub async fn update_samba_conf(
    ...
    for item in new_all_users.iter() {
        if !exist_system_user(item.user.as_str()) {
            add_user(item.user.as_str(), item.password.as_str())
                .map_err(into_smb_err!(SmbErrorCode::Failed, "add_user failed"))?;
        }
    }
