...
impl RepoServer {
    ...
    async fn get_need_chunk_in_remote_index_db(
        ...
        let meta_index_db =
            MetaIndexDb::new(new_meta_index_db_path.clone(), true).map_err(|e| {
                error!(
                    "open new meta-index-db {} failed, err:{}",
                    new_meta_index_db_path.to_string_lossy(),
                    e
                );
        ...
        for (pkg_id, _chunk_id) in pin_pkg_list.iter() {
            ...
            if pkg_meta.is_err() {
                ...
                error!(
                    "get pkg_meta failed from zone meta-index-db, err:{}",
                    err_string.as_str()
                );
            ...
            meta_index_db
                .cacl_pkg_deps_metas(&pkg_meta, &mut deps_metas)
                .map_err(|e| {
            ...
            if !pkg_meta.content.is_empty() {
                download_list.insert(
                    pkg_meta.content.clone(),
                    WillDownloadPkgInfo {
                        pkg_name: pkg_meta.name.clone(),
                        pkg_version: pkg_meta.version.clone(),
                        chunk_id: pkg_meta.content.clone(),
                        chunk_size: pkg_meta.size,
                    },
                );
            }
        }
    ...
    async fn try_create_wait_pub_meta_index_db(&self) -> Result<(), RPCErrors> {
        ...
        if !wait_pub_meta_index_db_path.exists() {
            info!(
                "Wait pub meta index db not exists, creating it by copying from pub meta index db"
            );
    ...
    async fn copy_file(target_file_path: &PathBuf, file_path: &PathBuf) -> Result<(), RPCErrors> {
        tokio::fs::copy(file_path, target_file_path)
            .await
            .map_err(|e| {
    ...
    async fn replace_file(
        ...
        // 执行重命名操作（相当于 move）
        tokio::fs::rename(file_path, target_file_path)
            .await
            .map_err(|e| {
    ...
    async fn pin_pkg_list(&self, pkg_list: &HashMap<String, String>) -> Result<(), RPCErrors> {
        ...
        // })?;
        // sys_config_client
        //     .set(&sys_config_path, &current_pkg_list_str)
        //     .await
        //     .map_err(|e| {
        ...
        //     named_mgr
        //         .set_file_impl(
        //             format!("/repo/install_pkg/{}", pkg_id).as_str(),
        //             &chunk_obj_id,
        //             "repo_service",
        //             "root",
        //         )
    ...
    async fn get_pin_pkg_list() -> Result<HashMap<String, String>, RPCErrors> {
        ...
        if current_pkg_list_str.is_err() {
            error!(
                "get pin pkg list failed, err:{}",
                current_pkg_list_str.err().unwrap()
            );
    ...
    async fn handle_install_pkg(&self, req: RPCRequest) -> Result<RPCResponse, RPCErrors> {
        ...
        // let pkg_list: HashMap<String, String> =
        //     serde_json::from_value(pkg_list.clone()).map_err(|e| {
        ...
        // let source_url = self.setting.remote_source.get("root");
        ...
        //         )));
        //     }
        //     let chunk_id = pkg_meta.content.clone();
        ...
        //     task_mgr_client
        //         .update_task_error(task_id, err_string.as_str())
        //         .await
        //         .map_err(|e| {
        ...
        //             info!(
        //                 "install pkg task pull chunk {} success, completed_size:{}",
        //                 chunk_id.to_string(),
        //                 completed_size
        //             );
        //             task_mgr_client
        //                 .update_task_progress(task_id, completed_size, total_size)
        //                 .await
        //                 .map_err(|e| {
    ...
    async fn do_sycn_from_remote_source(setting: &RepoServerSetting) -> Result<(), RPCErrors> {
        ...
        //     .map_err(|e| {
        //         error!(
        //             "download remote meta-index-db by {} failed, err:{}",
        //             root_source_url, e
        //         );
        ...
        //     return Err(RPCErrors::ReasonError(
        //         "check_new_remote_index failed".to_string(),
        //     ));
        // }

        // let need_check_chunk_list = need_check_chunk_list.unwrap();
        // let total_size = need_check_chunk_list
        //     .values()
        //     .map(|info| info.chunk_size)
        //     .sum::<u64>();
        // info!(
        //     "sync_from_remote_source, start check {} chunks,total size:{}",
        //     need_check_chunk_list.len(),
        //     total_size
        // );
        ...
        //         .set_file_impl(
        //             format!(
        //                 "/repo/install_pkg/{}/{}/chunk",
        //                 will_download_pkg_info.pkg_name, will_download_pkg_info.pkg_version
        //             )
        //             .as_str(),
        //             &chunk_id.to_obj_id(),
        //             "repo_service",
    ...
    async fn create_new_default_meta_index_db(user_id: &str) -> Result<(), RPCErrors> {
        ...
        //     RepoServer::copy_file(
        //         &default_meta_index_db_path,
        //         &default_source_meta_index_db_path,
        //     )
        //     .await?;
        //     have_result = true;
        ...
        //         let default_meta_index_db = MetaIndexDb::new(default_meta_index_db_path, false)
        //             .map_err(|e| {
        //                 error!("open default meta-index-db failed, err:{}", e);
        //                 RPCErrors::ReasonError(format!(
        //                     "open default meta-index-db failed, err:{}",
        //                     e
        //                 ))
        //             })?;
        //         default_meta_index_db
        //             .merge_meta_index_db(&pub_meta_index_db_path)
        //             .await
        //             .map_err(|e| {
        ...
        //     return Err(RPCErrors::ReasonError(
        //         "no record can write to meta-index-db ".to_string(),
        //     ));
        // }
    ...
    // TODO: 所有发布的内容，都要改成使用 publish-content-mgr 来处理
    // 直接使用ndm的set_file 操作可能会因为权限控制导致隐私数据泄露
    // ndm是zone使用的dfs,保存了所有的zone内数据（包括隐私数据)
    async fn handle_pub_pkg(&self, req: RPCRequest) -> Result<RPCResponse, RPCErrors> {
        ...
        */
        unimplemented!();
        // if !self.setting.enable_dev_mode {
        //     return Err(RPCErrors::ReasonError(
        //         "repo_server dev mode is not enabled".to_string(),
        //     ));
        // }
        ...
        // let pkg_meta_jwt_map: HashMap<String, String> = serde_json::from_value(pkg_list.clone())
        //     .map_err(|e| {
        //         error!("parse pkg_list failed, err:{}", e);
        //         RPCErrors::ReasonError(format!("parse pkg_list failed, err:{}", e))
        //     })?;
        ...
        //         error!(
        //             "verify pkg_meta_jwt failed, err:{}",
        //             verify_result.err().unwrap()
        //         );
        //         continue;
        //     }
        //     let pkg_meta_json = decode_json_from_jwt_with_default_pk(pkg_meta_jwt, &owner_pk)
        //         .map_err(|e| {
        ...
        //         tag: pkg_meta.version_tag.clone(),
    ...
    async fn handle_sign_index(&self, req: RPCRequest) -> Result<RPCResponse, RPCErrors> {
        ...
        // let (index_obj_id, _path_obj_jwt) =
        //     NamedDataMgr::get_obj_id_by_path(None, "/repo/pub_meta_index.db")
        //         .await
        //         .map_err(|e| {
        //             error!(
        //                 "get obj_id from ndn://repo/pub_meta_index.db failed, err:{}",
        //                 e
        //             );
    ...
    async fn handle_pub_pkg_to_source(&self, req: RPCRequest) -> Result<RPCResponse, RPCErrors> {
        ...
        */
        ...
        // let session_token = runtime.get_session_token().await;
        // let ndn_client = NdnClient::new(
        //     pub_task_data.author_repo_url.clone(),
        //     Some(session_token),
        //     None,
        // );
        ...
        //         continue;
        //     }
        //     let pkg_meta_json = decode_json_from_jwt_with_default_pk(pkg_meta_jwt, &author_pk)
        //         .map_err(|e| {
        //             error!("decode pkg_meta_jwt failed, err:{}", e);
        //             RPCErrors::ReasonError(format!("decode pkg_meta_jwt failed, err:{}", e))
        //         })?;
        ...
        // info!(
        //     "veify pub pkg_list ok, will download {} chunks, total size:{}",
        //     will_download_chunk_list.len(),
        //     total_size
        // );
        // task_mgr
        ...
        //     })?;
        // task_mgr
        //     .update_task_progress(task_id, 0, total_size)
        //     .await
        //     .map_err(|e| {
        //         error!("update task progress failed, err:{}", e);
        ...
        // task_mgr
        //     .update_task_status(task_id, TaskStatus::WaitingForApproval)
        //     .await
        //     .map_err(|e| {
        //         error!("update task status failed, err:{}", e);
        //         RPCErrors::ReasonError(format!("update task status failed, err:{}", e))
        ...
        for task in tasks.iter() {
            ...
            for (meta_obj_id, pkg_meta_jwt) in pub_task_data.pkg_list.iter() {
                ...
                let pkg_meta: PackageMeta =
                    serde_json::from_value(pkg_meta.clone()).map_err(|e| {
                        error!("parse pkg_meta_jwt failed, err:{}", e);
                        RPCErrors::ReasonError(format!("parse pkg_meta_jwt failed, err:{}", e))
                    })?;
