...
async fn service_main() -> Result<()> {
    ...
    if login_result.is_err() {
        error!(
            "repo service login to system failed! err:{:?}",
            login_result
        );
    ...
    let repo_service_settings = runtime.get_my_settings().await.map_err(|e| {
        error!("repo service settings not found! err:{}", e);
        anyhow::anyhow!("repo service settings not found! err:{}", e)
    })?;
    let repo_service_settings: RepoServerSetting = serde_json::from_value(repo_service_settings)
        .map_err(|e| {
    ...
    if !repo_server_data_folder.exists() {
        std::fs::create_dir_all(&repo_server_data_folder).map_err(|e| {
            error!(
                "Failed to create repo_server_data_folder: {}, err: {}",
                repo_server_data_folder.display(),
                e
            );
            anyhow::anyhow!(
                "Failed to create repo_server_data_folder: {}, err: {}",
        ...
        info!(
            "Created repo_server_data_folder: {}",
            repo_server_data_folder.display()
        );
    }

    let repo_server = RepoServer::new(repo_service_settings).await.map_err(|e| {
