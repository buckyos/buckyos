...
use buckyos_api::{
    ...
    BoxKind, BuckyOSRuntimeType, Contact, ContactQuery, Event, KEventClient, MsgCenterClient,
    MsgRecordWithObject, MsgState, SendContext, SystemConfigClient, UserState, UserType,
    ...
    async fn handle_chat_bootstrap(
        ...
        let response = ChatBootstrapResponse {
            ...
            capabilities: ChatCapabilityInfo {
                ...
                standalone_chat_app_link: false,
                opendan_channel_ready: false,
            },
            notes: vec![
                "Current control-panel chat wraps msg-center inside the desktop window shell."
                    .to_string(),
                "Browser traffic stays on /kapi/control-panel so auth and zone host routing are reused."
                    .to_string(),
                "Future standalone chat app and OpenDan control-channel integration remain follow-up work."
                    .to_string(),
            ],
        };
...
impl HttpServer for ControlPanelServer {
    async fn serve_request(
        ...
        if method == Method::POST && path == "/kapi/control-panel/chat/stream" {
