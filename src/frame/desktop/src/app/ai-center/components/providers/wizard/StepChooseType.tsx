...
interface StepChooseTypeProps {
  ...
  managedSnProviderEnabled: boolean
...
export function StepChooseType({ selected, onSelect, hasManagedSnProvider, managedSnProviderEnabled, catalog, loading, error, onRetry }: StepChooseTypeProps) {
  const { t } = useI18n()
  ...
  return <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">{choices.map((item) => {
    ...
    return <button key={item.type} type="button" onClick={() => { void onSelect(item.type) }} className="flex min-h-28 flex-col gap-2 rounded-xl p-4 text-left transition-all" style={{ background: active ? 'color-mix(in oklch, var(--cp-accent), transparent 90%)' : 'var(--cp-surface)', border: active ? '2px solid var(--cp-accent)' : '1px solid var(--cp-border)' }}><div className="flex items-center gap-2"><Icon size={19} style={{ color: active ? 'var(--cp-accent)' : 'var(--cp-muted)' }} /><span className="text-sm font-medium" style={{ color: 'var(--cp-text)' }}>{item.name}</span>{item.systemManaged && <span className="rounded px-1.5 py-0.5 text-[10px]" style={{ background: 'var(--cp-accent)', color: '#fff' }}>{hasManagedSnProvider && !managedSnProviderEnabled ? t('aiCenter.wizard.enable', 'Enable') : t('aiCenter.wizard.apiKey', 'API Key')}</span>}</div><span className="break-all text-xs" style={{ color: 'var(--cp-muted)' }}>{hasManagedSnProvider && !managedSnProviderEnabled && item.systemManaged ? t('aiCenter.wizard.snEnableExisting', 'Enable the SN Router from the current Zone SN account.') : item.description}</span></button>
  })}</div>
}
