...
import type { AppServiceItem, InstallTask } from '../types'
import { runtimeStatus } from '../mock/store'
...
function statusLabel(
  status: AppServiceItem['status'],
  t: ReturnType<typeof useI18n>['t'],
) {
  return t(`app22.status.${status}`, t('app22.unknown'))
}

function StatusBadge({ service }: { service: AppServiceItem }) {
  const { t } = useI18n()
  const color = statusColor(runtimeStatus(service.runtime))
  return (
    <span
    ...
    >
      <StatusIcon status={runtimeStatus(service.runtime)} />
      {statusLabel(runtimeStatus(service.runtime), t)}
    </span>
  )
}
  ...
  return (
    <button
      ...
      style={{
        ...
        boxShadow:
          '0 10px 28px color-mix(in srgb, var(--cp-shadow) 6%, transparent)',
      }}
    >
      <div className="flex w-full items-start gap-3.5">
        <div
          ...
          style={{
            color: 'var(--cp-text)',
            background:
              'color-mix(in srgb, var(--cp-accent-soft) 13%, var(--cp-surface-2))',
            border: '1px solid var(--cp-border)',
          }}
        >
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 truncate text-xs text-[var(--cp-muted)]">
            {t('app22.owner')}: {service.owner_user_id}
          </p>
          <p
    ...
    >
      {services.map((service, index) => (
        <div
          ...
          style={{
            background:
              index % 2 === 0
                ? 'var(--cp-surface)'
                : 'color-mix(in srgb, var(--cp-surface-2) 56%, var(--cp-surface))',
        ...
        >
          ...
          <p
            className="col-span-2 row-start-2 truncate text-xs sm:col-span-1 sm:col-start-2 sm:row-start-1"
            style={{ color: 'var(--cp-muted)' }}
          >
            {t(service.description)}
          </p>
...
function ActiveTaskBanner({
  ...
  return (
    <section
      ...
      className="flex flex-wrap items-center justify-between gap-3 rounded-[20px] border border-[var(--cp-border)] bg-[var(--cp-surface)] p-4"
    >
      <div className="min-w-0 flex-1">
        <h2 className="text-sm font-semibold">
          {task.app.show_name} · {task.owner_user_id}
        </h2>
      ...
      <button
        type="button"
        onClick={onOpen}
        className="min-h-11 rounded-xl bg-[var(--cp-accent)] px-4 text-sm font-semibold text-[var(--cp-surface)]"
      >
        {t('app22.viewTask')}
      </button>
    </section>
...
function LoadingState() {
  ...
  return (
    <div
      className="space-y-6"
      aria-label={t('appService.state.loading', 'Loading application services')}
    >
      <div
        className="h-24 animate-pulse rounded-[20px]"
      ...
      <div className="grid gap-3 md:grid-cols-2">
        {[0, 1, 2, 3].map((item) => (
          <div
            key={item}
            className="h-40 animate-pulse rounded-[20px]"
            style={{ background: 'var(--cp-surface)' }}
          />
        ))}
      </div>
...
function ErrorState({ onRetry }: { onRetry: () => void }) {
  ...
  return (
    <div className="mx-auto flex max-w-xl flex-col items-start py-16">
      ...
      >
        {t(
          'appService.state.errorBody',
          'The runtime snapshot could not be loaded. Retry without changing any application state.',
        )}
      </p>
      <button
        type="button"
...
function EmptyApps({ onAdd }: { onAdd: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        {t(
          'appService.state.emptyBody',
          'Add an App DID, App Meta URL, or .pikg package to begin managing it here.',
        )}
      </p>
      <button
        type="button"
...
export function HomePage({ onNavigate, onOpenInstaller }: HomePageProps) {
  ...
  return (
    <div className="space-y-8">
      <header className="flex items-start justify-between gap-5">
        <div className="min-w-0">
          ...
          >
            {t(
              'appService.subtitle',
              'Install applications, observe runtime state, and understand failures.',
            )}
          </p>
        </div>
        <button
      ...
      {store.viewStatus === 'error' && (
        <ErrorState onRetry={() => store.retryLoad()} />
      )}
      ...
      {store.viewStatus === 'ready' && (
        <>
          ...
          {Object.values(store.drafts).map((draft) => (
            <section
              key={draft.draft_id}
              className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-[var(--cp-border)] p-4"
            >
              <span className="min-w-0 break-all text-sm">
                {draft.app.show_name} · {t('app22.draftHint')}
              </span>
              <button
                type="button"
                className="min-h-11 rounded-xl border border-[var(--cp-border)] px-4"
                onClick={() =>
                  void onOpenInstaller({ draft_id: draft.draft_id })
                }
              >
                {t('app22.restoreDraft')}
              </button>
            </section>
          ))}
          {store.getTasks().map((task) => (
            <ActiveTaskBanner
              key={task.task_id}
              task={task}
              onOpen={() => void onOpenInstaller({ task_id: task.task_id })}
            />
          ))}
          ...
          <section>
            <SectionHeading
              ...
              count={apps.length}
              description={t(
                'appService.layer.appsDescription',
                'Installed apps and their current runtime state.',
              )}
            />
            {apps.length === 0 ? (
              ...
              <div className="grid gap-3 md:grid-cols-2">
                {apps.map((service) => (
                  <AppCard
                    ...
                    service={service}
                    onOpen={() =>
                      onNavigate({ page: 'detail', serviceId: service.id })
                    }
                  />
                ))}
              </div>
          ...
          <section>
            <SectionHeading
              ...
              description={t(
                'appService.layer.systemDescription',
                'Zone services that support applications and system state.',
              )}
            />
          ...
          <section>
            <SectionHeading
              ...
              description={t(
                'appService.layer.kernelDescription',
                'Low-level node services are shown as read-only runtime information.',
              )}
            />
