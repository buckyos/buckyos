...
test('desktop layout restores AI Center launcher entry and opens panel content', async ({
  ...
  await expect(page.getByText('https://metadata.example/aicc/driver-metadata/index.json')).toBeVisible()
  await expect(page.getByText('Replace with', { exact: true })).toBeVisible()
  await expect(page.getByLabel('Metadata source URL')).toHaveValue('')
  await expect(page.getByLabel('Metadata source URL')).toHaveAttribute('placeholder', 'Enter a new source URL')
