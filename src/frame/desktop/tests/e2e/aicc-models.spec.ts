...
test('catalog cards, specifications, details and combined filters', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('[data-model="gpt-5.1"]').getByRole('img')).toHaveCount(0)
  await expect(page.locator('[data-model="qwen3.5-27b"]').getByRole('img', { name: 'Locally deployable', exact: true })).toBeVisible()
  await expect(page.locator('[data-model="qwen2.5-coder-32b"]').getByRole('img', { name: 'Deployed locally', exact: true })).toBeVisible()
  const openai = page.getByRole('region', { name: 'OpenAI', exact: true })
  const specButton = openai.getByRole('button', { name: 'gpt-standard, 2 models', exact: true })
  await expect(specButton).toHaveAttribute('aria-expanded', 'false')
  const collapse = openai.getByRole('button', { name: 'Collapse OpenAI', exact: true })
  await collapse.click()
  await expect(openai.getByTestId('model-card').first()).toBeHidden()
  await expect(specButton).toBeHidden()
  await openai.getByRole('button', { name: 'Expand OpenAI', exact: true }).click()
  await expect(openai.getByTestId('model-card').first()).toBeVisible()
  await specButton.click()
  const spec = openai.getByRole('region', { name: 'gpt-standard, 2 models', exact: true })
  ...
  await page.getByRole('button', { name: 'Clear filters' }).click()
  await page.getByRole('button', { name: 'gpt-standard, 2 models', exact: true }).click()
...
test('catalog remains browsable without providers', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'qwen-code, 0 models', exact: true }).click()
...
test('refresh lights the local indicator only after local inventory appears', async ({ page }) => {
  ...
  await expect(card.getByRole('img', { name: 'Locally deployable', exact: true })).toBeVisible()
  ...
  await expect(card.getByRole('img', { name: 'Deployed locally', exact: true })).toBeVisible()
  await page.getByRole('checkbox', { name: 'Deployed locally only', exact: true }).check()
