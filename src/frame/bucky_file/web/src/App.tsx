import type { ChangeEventHandler, DragEvent, FormEvent } from 'react'
...
const App = () => {
  ...
  const [mainTab, setMainTab] = useState<MainTab>(() => getMainTabFromPathname(window.location.pathname))
  ...
  const [publicSharePassword, setPublicSharePassword] = useState(() => getSearchParam('password'))
  ...
  const [publicSharePath, setPublicSharePath] = useState(() => normalizeUrlPath(getSearchParam('path') || '/'))
  ...
  useEffect(() => {
    if (!token || publicShareId || mainTab !== 'files') {
      return
    }
    void loadDirectory(currentPath, token)
  }, [token, currentPath, loadDirectory, mainTab, publicShareId])
  ...
  }, [locationPathname, mainTab, publicShareId])
  ...
  useEffect(() => {
    ...
    void loadShares(token)
  }, [token, loadShares, clearPreviewState, clearSearchState])
  ...
  }, [clearSearchState, loadPublicShare])
  ...
  const onLogin = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!jwt) {
        setMessage('Login failed: empty token.')
        return
      }
    ...
    try {
      ...
      const workers = Array.from({ length: workerCount }, async () => {
        while (queueIndex < files.length) {
          ...
          const result = await performUploadFile(token, currentFile, targetPath, progressKey, resumeKey)
          if (result.ok) {
            completedCount += 1
      ...
      clearSearchState()
      await loadDirectory(currentPath, token)
  ...
  const onCancelUpload = async (key: string) => {
    ...
    if (sessionId) {
      try {
        await deleteUploadSession(token, sessionId)
      } catch {
  ...
  const onRetryUpload = async (key: string) => {
    ...
    setLoading(true)
    try {
      const result = await performUploadFile(token, task.file, task.targetPath, key, task.resumeKey)
      if (result.ok) {
        clearSearchState()
        await loadDirectory(currentPath, token)
  ...
  const downloadQuery = useMemo(() => encodeURIComponent(token), [token])
  ...
  if (!token) {
    return (
      <main className="bucky-file-app flex min-h-screen items-center justify-center bg-[radial-gradient(circle_at_top,#d7ece8,transparent_55%),#f4f8f7] px-4">
        <section className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <p className="mt-1 text-sm text-slate-600">Sign in to access your files.</p>
          <form className="mt-6 space-y-4" onSubmit={onLogin}>
            <label className="block text-sm text-slate-700">
              ...
              <input
                ...
                className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20"
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full rounded-xl bg-primary px-4 py-2 text-sm font-semibold text-white transition hover:bg-teal-700 disabled:cursor-not-allowed disabled:opacity-60"
            >
              <span className="inline-flex items-center gap-1.5">
                <Icon name="signin" />
  ...
  return (
    <main className="bucky-file-app min-h-screen bg-[radial-gradient(circle_at_top,#d9eeea,transparent_58%),#f4f8f7] px-3 py-4 md:px-6 md:py-6">
      <div className="mx-auto w-full max-w-[1280px] space-y-4">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="rounded-lg border border-rose-300 px-3 py-2 text-sm font-medium text-rose-600 transition hover:bg-rose-50"
              >
                <span className="inline-flex items-center gap-1.5">
                  <Icon name="signout" />
                  Sign out
        ...
        <section className="overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-sm">
          {mainTab === 'files' ? (
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-xs uppercase tracking-[0.15em] text-slate-500">My files</p>
          ...
          {currentPathIsDir ? (
            <>
              ...
              <div
                className={`relative overflow-x-auto px-3 py-3 transition md:px-4 ${
                        ...
                        <tr key={entry.path} className="text-slate-800">
                          ...
                          <td className="border-b border-slate-100 px-3 py-2">
                            <div className="flex items-center gap-2">
                              {!entry.is_dir ? (
                                <a
                                  href={`/api/raw${encodePath(entry.path)}?auth=${downloadQuery}`}
                                  className="rounded border border-slate-300 px-2 py-1 text-xs font-semibold text-slate-700 transition hover:border-primary hover:text-primary"
                                >
                                  <span className="inline-flex items-center gap-1.5">
                                    <Icon name="download" />
                              ...
                              {!entry.is_dir ? (
                                <button
                                  type="button"
                                  onClick={() => void onOpenEditor(entry)}
                                  className="rounded border border-slate-300 px-2 py-1 text-xs font-semibold text-slate-700 transition hover:border-primary hover:text-primary"
                                >
                                  <span className="inline-flex items-center gap-1.5">
                                    <Icon name="open" />
                              ...
                              <button
                                type="button"
                                onClick={() => void onMoveOrCopy(entry, 'move')}
                                className="rounded border border-slate-300 px-2 py-1 text-xs font-semibold text-slate-700 transition hover:border-primary hover:text-primary"
                              >
                                <span className="inline-flex items-center gap-1.5">
                                  <Icon name="move" />
                              ...
                              <button
                                type="button"
                                onClick={() => void onMoveOrCopy(entry, 'copy')}
                                className="rounded border border-slate-300 px-2 py-1 text-xs font-semibold text-slate-700 transition hover:border-primary hover:text-primary"
                              >
                                <span className="inline-flex items-center gap-1.5">
                                  <Icon name="copy" />
                              ...
                              <button
                                type="button"
                                onClick={() => void onCreateShare(entry)}
                                className="rounded border border-amber-300 px-2 py-1 text-xs font-semibold text-amber-700 transition hover:bg-amber-50"
                              >
                                <span className="inline-flex items-center gap-1.5">
                                  <Icon name="share" />
                                  Share
        ...
        <div className="fixed bottom-5 right-5 z-40 flex max-h-[78vh] w-[min(92vw,420px)] flex-col items-end gap-2">
          {uploadPanelOpen ? (
