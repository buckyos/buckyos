...
处理规则:`harness/process_ruels/dev_task.md`

### 规划任务

判别条件:当任务引用`proposals/$feature_name` 目录开始。
处理规则:`harness/process_ruels/plan_proposal.md`

### FixBug

判别条件:当任务内容是解决一个bug,或则引用一个内容是BUG描述的url时。
处理规则:`harness/process_ruels/fixbug.md`


### 重构

判别条件:当任务内容是“整理现有实现/提取共用代码"等经典重构方向时
处理规则:`harness/process_ruels/refactor.md`

### 其它口头开发任务

处理规则:`harness/process_ruels/others.md`
