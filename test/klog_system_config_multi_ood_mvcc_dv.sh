...
TEST_TARGET_DIR="${REPO_ROOT}/src/target/test_klog_system_config_multi_ood_mvcc_dv"

cd "${REPO_ROOT}"

export BUCKYOS_ROOT="${BUCKYOS_ROOT:-${REPO_ROOT}/.dev_buckyos}"
export KLOG_CLUSTER_DV_MODE="local-gateway-system-config-multi-ood-mvcc"
...
if [[ -z "${SYSTEM_CONFIG_BIN:-}" ]]; then
  echo "[build] cargo build -p system_config --bin system_config"
  (cd "${REPO_ROOT}/src" && cargo build -p system_config --bin system_config)
