...
TEST_TARGET_DIR="${REPO_ROOT}/src/target/test_klog_system_config_rollout_dv"

cd "${REPO_ROOT}"

export BUCKYOS_ROOT="${BUCKYOS_ROOT:-${REPO_ROOT}/.dev_buckyos}"
export KLOG_CLUSTER_DV_MODE="local-gateway-system-config-rollout"
...
echo "[run] cargo build -p system_config -p klog_daemon"
