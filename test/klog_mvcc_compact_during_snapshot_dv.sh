...
TEST_TARGET_DIR="${REPO_ROOT}/src/target/test_klog_mvcc_compact_during_snapshot_dv"

cd "${REPO_ROOT}"

export BUCKYOS_ROOT="${BUCKYOS_ROOT:-${REPO_ROOT}/.dev_buckyos}"
export KLOG_CLUSTER_DV_MODE="local-gateway-mvcc-compact-during-snapshot"
...
echo "[diag] KLOG_MVCC_COMPACT_DURING_SNAPSHOT_KEYS=${KLOG_MVCC_COMPACT_DURING_SNAPSHOT_KEYS:-<default>}"
echo "[diag] KLOG_MVCC_COMPACT_DURING_SNAPSHOT_VALUE_BYTES=${KLOG_MVCC_COMPACT_DURING_SNAPSHOT_VALUE_BYTES:-<default>}"
echo "[diag] KLOG_MVCC_COMPACT_DURING_SNAPSHOT_CHUNK_BYTES=${KLOG_MVCC_COMPACT_DURING_SNAPSHOT_CHUNK_BYTES:-<default>}"
