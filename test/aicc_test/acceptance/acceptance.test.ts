...
import { ASSET_LABEL, SCENARIOS } from "../../jarvis_media_dv/scenarios.ts";
import {
  buildEntryCoverage,
  productDefects,
  type StepResult,
...
function t15ProviderRequest(
  ...
  if (contract.auth.kind === "header") {
    ...
    url.searchParams.set(
      contract.auth.name,
      `${contract.auth.prefix}t15-secret`,
    );
  ...
  if (contract.content_type === "multipart/form-data") {
    ...
    for (const [field, value] of Object.entries({ ...fields, ...extraBody })) {
      if (value && typeof value === "object") {
        form.append(field, new Blob(["t15"]), `${field}.bin`);
      } else form.append(field, String(value));
    }
    return {
      url: `${baseUrl}${path}`,
      init: { method: contract.http_method, headers, body: form },
    };
  }
  ...
  return {
    ...
    init: {
      ...
      headers,
      body: JSON.stringify({ ...fields, ...extraBody }),
    },
  };
}
...
test("cloud update fixtures replace complete catalog files and tombstone every cloud identity", async () => {
  ...
  assert.deepEqual(
    first.map((file) => `${file.catalog_kind}:${file.catalog_id}`),
    [
      "model_driver:openai",
  ...
  const firstModels = first[0].contents.models as Array<
    Record<string, unknown>
  >;
  const secondModels = second[0].contents.models as Array<
    Record<string, unknown>
  >;
  assert.equal(
    firstModels.some((model) => model.id === "text-embedding-3-small"),
    false,
  );
  assert.equal(
    secondModels.some((model) => model.id === "text-embedding-3-small"),
    true,
  );
  assert.ok(
  ...
  assert.deepEqual(
    cloudUpdateTombstones(44).map((item) =>
      `${item.catalog_kind}:${item.catalog_id}`
    ),
    [
      "model_driver:openai",
      "provider_rules:aicc-cloud-update-openai",
...
test("Judge verdict parser enforces the requested strict schema", () => {
  assert.deepEqual(
    parseJudgeVerdict('{"pass":true,"score":0.9,"reason":"meets rubric"}', 0.8),
    {
      passed: true,
      score: 0.9,
      reason: "meets rubric",
  ...
  assert.throws(() =>
    parseJudgeVerdict('{"pass":true,"score":0.9,"reasoning":"ok"}', 0.8)
  );
  assert.throws(() =>
  ...
  assert.deepEqual(
    outputResources({
      ...
      result: {
        artifacts: [{
          mime: "audio/mpeg",
          resource: { kind: "named_object", obj_id: "chunk:test" },
        }],
      },
    }),
    [{
...
test("canonical requirements preserve separate api_type and method value sets", async () => {
  const source = await readFile(
    join(here, "../../../doc/aicc/aicc_e2e_test_requirements.md"),
    "utf8",
  );
...
function matrixInputs(inventories: ProviderInventory[]) {
  return {
    officialInventories: structuredClone(inventories),
    aiccInventories: inventories,
  };
}
...
test("SN official catalog requires an independent bearer session token", async () => {
  const profile = (await baseline()).providers.find((item) =>
    item.provider_driver === "sn-ai-provider"
  )!;
  ...
  await assert.rejects(
    fetchOfficialModelIds({
      ...
      fetcher: async () =>
        new Response(
          JSON.stringify({
            models: profile.official_catalog.endpoint_ids!.slice(1).map((
              endpoint_id,
            ) => ({ endpoint_id })),
            has_more: false,
          }),
...
test("official catalog network failures do not expose query credentials", async () => {
  const profile = (await baseline()).providers.find((item) =>
    item.provider_driver === "google-gemini"
  )!;
  await assert.rejects(
    fetchOfficialModelIds({
      profile,
...
test("official catalog is the inventory baseline and exposes AICC omissions", async () => {
  ...
  assert.ok(
    result.mismatches.includes(
      "official_supported_but_aicc_missing openai/gpt-6",
    ),
  );
  assert.ok(
    result.cells.some((cell) => cell.provider_model_id === "gpt-5.6-sol"),
  );
...
test("official catalog coverage rules remove logical aliases and deduplicate physical models", async () => {
  ...
  const openai = providerBaseline.providers.find((item) =>
  ...
  assert.deepEqual(
    result.inventories[0].models.map((model) => model.provider_model_id),
    ["gpt-5.6-sol"],
  );
  assert.equal(
    result.coverage.find((item) => item.provider_model_id === "gpt-current")
      ?.reason,
    "duplicate_physical_model",
  );
  assert.equal(
    result.coverage.find((item) => item.provider_model_id === "gpt-cheapest")
      ?.reason,
    "logical_alias",
  );
});
...
test("T3 manifest includes six inbound kinds and multi-attachment history", () => {
  const kinds = new Set([
    "message-text",
    ...Object.values(ASSET_LABEL).map((mime) =>
  ...
  const multi = SCENARIOS.find((scenario) =>
    scenario.id === "multi_attachment_current_and_history"
  );
  assert.ok(multi);
  assert.ok(multi.steps.some((step) => (step.attachments?.length ?? 0) >= 3));
  assert.ok(multi.steps.some((step) => Boolean(step.replyToStep)));
  const videoPackage = SCENARIOS.find((scenario) =>
    scenario.id === "multi_output_video_subtitle_cover"
  );
  assert.deepEqual(videoPackage?.steps[0].expect.artifacts, [
    "video/",
    "text/",
    "image/",
  ]);
  assert.deepEqual(videoPackage?.steps[0].expect.attachmentCount, {
    min: 3,
    max: 3,
  });
  assert.equal(
    SCENARIOS.find((scenario) => scenario.id === "document_vector_retrieval")
      ?.coverage?.status,
    "not_applicable",
  );
  for (
    ...
    assert.ok(
      scenario.requiredAssets.some((asset) => asset.startsWith("archive_")),
    );
  }
  assert.equal(
    SCENARIOS.find((scenario) =>
      scenario.id === "duplicate_inbound_idempotency"
    )
  ...
  assert.equal(
    SCENARIOS.find((scenario) =>
      scenario.id === "outbound_delivery_idempotency"
    )
      ?.steps[0].assertUniqueOutbound,
    true,
  );
  assert.equal(
    SCENARIOS.find((scenario) => scenario.id === "group_message_semantics")
      ?.requiresGroup,
    true,
  );
  assert.equal(
    SCENARIOS.find((scenario) => scenario.id === "forwarded_message_source")
      ?.steps[0].sourceDid,
...
test("T3 report records product assertions with expected, observed, and evidence", () => {
  ...
  const defects = productDefects([
    base,
    {
      ...base,
      step_id: "transport",
      failure_class: "message_transport_failed",
    },
  ]);
...
test("T1 mock settings append run-scoped instances without mutating backup", () => {
  const original = {
    providers: [{
      provider_instance_name: "production",
  ...
  assert.deepEqual(
    providers.slice(1, 3).map((item) => item.provider_instance_name),
    [
      "dv-openai-a-run-one",
      "dv-openai-b-run-one",
    ],
  ...
  assert.equal(
    (patched.session_config as { revision: string }).revision,
    "dv-routing-run-one",
  );
...
test("provider scheduler runs sessions concurrently within global and provider limits", async () => {
  ...
  const results = await scheduler.run(items, async (item) => {
    ...
    providerPeaks.set(
      item.provider_driver,
      Math.max(providerPeaks.get(item.provider_driver) ?? 0, active),
    );
  ...
  assert.ok(
    ...
    "execution unexpectedly became fully serial",
  );
});

test("provider scheduler enforces request start interval per provider", async () => {
  const scheduler = new ProviderScheduler(4, {
    maxConcurrency: 3,
    minIntervalMs: 30,
  });
  const starts: number[] = [];
  ...
  for (let index = 1; index < starts.length; index += 1) {
    assert.ok(
      starts[index] - starts[index - 1] >= 24,
      `start interval was ${starts[index] - starts[index - 1]}ms`,
    );
  }
});
    ...
    extractFinance({
      result: {
        ...
        extra: {
          billing: {
            raw_cost_usd: 0.05,
            sn_ai_provider_credit_applied_usd: 0.03,
          },
        },
      },
    }),
...
test("T3 provider audit derives driver coverage from exact models and runtime inventory", () => {
  const coverage = providerCoverage({
    exactModels: [
      "vendor@model@openai-main",
      "claude-4@claude-main",
      "malformed",
    ],
    inventories: [
...
test("T1 manifest coverage does not count declared but unexecuted cases", () => {
  ...
  assert.ok(
    coverage.unexecuted_case_ids.includes(
...
test("T1 report separates requirement branches from combination cells", () => {
  ...
  const selected = manifest.find((item) =>
    item.case_id === "t1.task.concurrent_idempotency"
  );
  assert.ok(selected);
  ...
  const branch = coverage.branches.find((item) =>
    item.branch_id === "task.concurrent_idempotency"
  );
  assert.equal(branch?.status, "failed");
  assert.ok(coverage.total_branches > 50);
  assert.equal(
    coverage.combination_groups.find((item) =>
      item.group_id === "cross_cutting"
    )?.executed_cells,
    1,
  );
  assert.equal(
    coverage.combination_groups.find((item) =>
      item.group_id === "canonical_api_routes"
    )?.executed_cells,
    0,
  );
});
...
test("T1 requirement coverage keeps optional credential skips distinct from failures", () => {
  const manifest = buildStaticManifest();
  const selected = manifest.find((item) =>
    item.case_id === "t1.security.cross_tenant"
  );
  assert.ok(selected);
  const coverage = buildT1Coverage(manifest, [{
    run_id: "run",
  ...
  assert.equal(
    coverage.branches.find((item) => item.branch_id === "security.cross_tenant")
      ?.status,
    "skipped",
  );
...
test("manifest rejects an illegal method and api_type association", () => {
  const testCase = structuredClone(
    buildStaticManifest().find((item) => item.api_type === "image.txt2img")!,
  );
  testCase.method = "image.txt2img";
  assert.throws(
    () => validateCaseManifest([testCase]),
    /method is not valid for api_type/,
  );
});
...
test("T2 LLM output variants build and assert JSON schema and tool-call contracts", () => {
  ...
  const jsonRequest = buildExactRequest({
    cell: jsonCell,
    runId: "run",
    fixtures: {},
  });
  assert.deepEqual(jsonRequest.requirements, {
    must_features: ["json_output"],
    resp_format: "json",
  });
  assert.doesNotThrow(() =>
    assertResponseShape(jsonCell, {
      task_id: "task",
      status: "succeeded",
      result: {
        message: {
          role: "assistant",
          content: [{ type: "text", text: '{"marker":"BUCKYOS-AICC-4827"}' }],
        },
        usage: {},
        cost: {},
      },
  ...
  const toolRequest = buildExactRequest({
    cell: toolCell,
    runId: "run",
    fixtures: {},
  });
  assert.equal(
    ((((toolRequest.payload as Record<string, unknown>).input_json as Record<
      string,
      unknown
    >)
...
test("typed inference adapter removes the legacy request envelope", async () => {
  ...
  assert.equal(
    (calls[0].params.messages as Array<{ content: unknown[] }>)[0].content
      .length,
    2,
  );
...
test("active official model families are not excluded by lifecycle filters", async () => {
  ...
  const rules = (driver: string) =>
    providerBaseline.providers
      .find((provider) => provider.provider_driver === driver)
      ?.coverage_rules ?? [];
  assert.equal(
    rules("openai").some((rule) => rule.model_pattern === "gpt-5.5*"),
    false,
  );
  assert.equal(
    rules("claude").some((rule) => rule.model_pattern === "claude-opus-4-*"),
    false,
  );
  assert.equal(
    rules("google-gemini").some((rule) =>
      ["gemini-2.5-*", "gemini-3.1-*", "gemini-3.5-*", "gemini-3.6-*"].includes(
        rule.model_pattern,
      )
    ),
    false,
  ...
  for (const driver of ["google-gemini", "fal"]) {
    assert.equal(
      providerBaseline.providers.find((provider) =>
        provider.provider_driver === driver
      )
        ?.rules.some((rule) =>
          rule.model_pattern === "*" && rule.status === "removed"
        ),
      false,
    );
  }
...
test("T2 Veo extension requests the protocol-fixed seven second duration", () => {
  const request = buildExactRequest({
    ...
    runId: "run",
    fixtures: {
      video: { kind: "base64", mime: "video/mp4", data_base64: "AA==" },
    },
  });
  assert.equal(
    ((request.payload as Record<string, unknown>).input_json as Record<
      string,
      unknown
    >).duration_seconds,
    7,
...
test("physical model coverage excludes lifecycle and logical aliases while retaining variants", async () => {
    ...
    inventories: [
      {
        ...
        models: [
          {
            exact_model: "gpt-image-1@openai-default",
            provider_model_id: "gpt-image-1",
            api_types: ["image.txt2img"],
            logical_mounts: [],
          },
          {
            exact_model: "sora-2@openai-default",
            provider_model_id: "sora-2",
            api_types: ["video.txt2video"],
            logical_mounts: [],
          },
          {
            exact_model: "gpt-3.5-turbo@openai-default",
            provider_model_id: "gpt-3.5-turbo",
            api_types: ["llm"],
            logical_mounts: [],
          },
          {
            exact_model: "gpt-5.1-codex-mini@openai-default",
            provider_model_id: "gpt-5.1-codex-mini",
            api_types: ["llm"],
            logical_mounts: [],
          },
          {
            exact_model: "o4-mini@openai-default",
            provider_model_id: "o4-mini",
            api_types: ["llm"],
            logical_mounts: [],
          },
          {
            exact_model: "gpt-5.6-sol@openai-default",
            provider_model_id: "gpt-5.6-sol",
            api_types: ["llm"],
            logical_mounts: [],
          },
          {
            exact_model: "gpt-5.6-sol:reasoning-high@openai-default",
            provider_model_id: "gpt-5.6-sol:reasoning-high",
            provider_actual_model_id: "gpt-5.6-sol",
            api_types: ["llm"],
            logical_mounts: [],
          },
        ],
      },
      {
        ...
        models: [
          {
            exact_model: "gemini-flash-latest@gemini-default",
            provider_model_id: "gemini-flash-latest",
            api_types: ["llm"],
            logical_mounts: [],
          },
          {
            exact_model: "gemini-3.7-flash@gemini-default",
            provider_model_id: "gemini-3.7-flash",
            api_types: ["llm"],
            logical_mounts: [],
          },
  ...
  assert.deepEqual(
    result.inventories[1].models.map((model) => model.provider_model_id),
    ["gemini-3.7-flash"],
  );
  assert.equal(
    result.coverage.find((item) => item.provider_model_id === "gpt-image-1")
      ?.reason,
    "deprecated_or_retiring",
  );
  assert.equal(
    result.coverage.find((item) => item.provider_model_id === "sora-2")?.reason,
    "deprecated_or_retiring",
  );
  assert.equal(
    result.coverage.find((item) => item.provider_model_id === "gpt-3.5-turbo")
      ?.reason,
    "deprecated_or_retiring",
  );
  assert.equal(
    result.coverage.find((item) =>
      item.provider_model_id === "gpt-5.1-codex-mini"
    )?.reason,
    "deprecated_or_retiring",
  );
  assert.equal(
    result.coverage.find((item) => item.provider_model_id === "o4-mini")
      ?.reason,
    "deprecated_or_retiring",
  );
  assert.equal(
    result.coverage.find((item) =>
      item.provider_model_id === "gemini-flash-latest"
    )?.reason,
    "logical_alias",
  );
  assert.equal(
    result.coverage.find((item) =>
      item.provider_model_id.includes("reasoning-high")
    )?.status,
    "included",
  );
});
...
test("T2 Gemini Embedding 2 has one minimal cell per API type and no variant cells", async () => {
  ...
  assert.deepEqual(cells.map((cell) => cell.api_type).sort(), [
...
test("T1.5 protocol catalog is independent, traceable, and strict on Provider wire", async () => {
  ...
  assert.deepEqual(
    validateProviderRequest(contract, {
      method: "POST",
  ...
  assert.ok(
    claudeToolErrors.some((error) =>
      error.includes("immediately return every preceding Claude tool_use")
    ),
  );
  const geminiInteractions = protocolContract(
    catalog,
    "google-gemini",
  ...
  assert.ok(
    geminiToolErrors.some((error) =>
      error.includes("content cannot contain Gemini interaction steps")
    ),
  );
  assert.ok(
    geminiToolErrors.some((error) =>
  ...
  assert.ok(
    geminiConfigErrors.some((error) =>
      error.includes("generation_config.temperature")
    ),
  );
  assert.ok(
    geminiConfigErrors.some((error) =>
      error.includes("generation_config.candidate_count")
    ),
  );
  assert.ok(
    geminiConfigErrors.some((error) =>
      error.includes("typed Gemini response format")
    ),
  );
  ...
  assert.ok(
    chatToolErrors.some((error) =>
      error.includes("tool_call_id must be a non-empty string")
    ),
  );
  assert.deepEqual(
    validateProviderRequest(contract, {
  ...
  assert.deepEqual(
    ...
    [
      "body field messages has invalid type",
      "body field max_tokens has invalid type",
    ],
  );
  assert.deepEqual(
    validateProviderRequest(contract, {
  ...
  const openaiResponses = protocolContract(
    catalog,
    "openai",
    "openai.responses.v1",
  );
  assert.deepEqual(
    validateProviderRequest(openaiResponses, {
      method: "POST",
  ...
  assert.ok(
    contract.official_sources.every((source) => source.startsWith("https://")),
  );
  const sn = protocolContract(
    catalog,
    "sn-ai-provider",
    "sn.openai-responses.v1",
  );
  assert.equal(sn.path, "/api/v1/ai/responses");
  assert.ok(
    sn.official_sources.every((source) =>
      source.includes("buckyos/sn-business/blob/f765081")
    ),
  );
  assert.equal(
    catalog.providers.find((provider) => provider.provider_driver === "qwen")
      ?.instance_fields?.workspace,
    "t15-workspace",
  );
  const selected = selectOfficialModels(catalog, "google-gemini", "seed-4827");
  assert.deepEqual(
    selected,
    selectOfficialModels(catalog, "google-gemini", "seed-4827"),
  );
  const gemini = catalog.providers.find((provider) =>
    provider.provider_driver === "google-gemini"
  )!;
  for (const [apiType, modelId] of Object.entries(selected)) {
    assert.ok(
      gemini.official_first_party_model_ids?.[apiType].includes(modelId),
    );
  }
  const invalidCatalog = structuredClone(catalog);
  invalidCatalog.providers[0].contracts[0].official_sources = [
    "https://example.com/not-provider-evidence",
  ];
  assert.throws(
  ...
  assert.ok(
    catalog.providers.flatMap((provider) => provider.contracts)
      .every((candidate) =>
        validateProviderSuccessFixture(candidate).length === 0
      ),
  );
  const invalidGeminiFixture = structuredClone(
    protocolContract(catalog, "google-gemini", "gemini.interactions.v1beta"),
  );
  invalidGeminiFixture.success_fixture = {
    id: "wrong",
    outputs: [{ type: "text", text: "wrong" }],
  };
...
test("T1.5 Provider mock rejects non-official wire and redacts captured credentials", async (context) => {
  ...
  assert.ok(
    ((await discovery.json()) as { data: Array<{ id: string }> }).data.some((
      model,
    ) => model.id === "gpt-5.6"),
  );
  assert.equal(
  ...
  assert.ok(
    Array.isArray(((await snDiscovery.json()) as { items: unknown[] }).items),
  );
  const select = async () => {
...
test("T1.5 Provider mock implements each machine discovery contract and rejects catalog-only discovery", async (context) => {
  ...
  for (const provider of catalog.providers) {
    ...
    if (discovery.mode === "catalog_only") {
      assert.equal(
        (await fetch(`${baseUrl}${provider.endpoint_path}/models`)).status,
        404,
    ...
    assert.equal(
      ...
      401,
      `${provider.provider_driver} unauthenticated discovery`,
    );
    const url = new URL(`${baseUrl}${discovery.path}`);
    for (
      const [name, value] of Object.entries(discovery.required_query ?? {})
    ) url.searchParams.set(name, value);
    const headers = new Headers(discovery.required_headers ?? {});
    if (contract.auth.kind === "header") {
      headers.set(contract.auth.name, `${contract.auth.prefix}t15-secret`);
    } else {url.searchParams.set(
        contract.auth.name,
        `${contract.auth.prefix}t15-secret`,
      );}
    ...
    assert.equal(
      response.status,
      200,
      `${provider.provider_driver} discovery: ${await response.clone().text()}`,
    );
...
test("T1.5 Provider mock serves every declared streaming and official error fixture", async (context) => {
  ...
  for (const provider of catalog.providers) {
    ...
    for (const fixture of catalog.error_fixtures[provider.provider_driver]) {
      ...
      assert.equal(
        (await fetch(providerRequest.url, providerRequest.init)).status,
        fixture.status,
        `${provider.provider_driver}/${fixture.scenario}`,
      );
    }
    for (
      const contract of provider.contracts.filter((candidate) =>
        candidate.stream_protocol
      )
    ) {
      assert.equal(
        (await fetch(`${baseUrl}/__mock/select`, {
      ...
      assert.equal(
        response.status,
        200,
        `${provider.provider_driver}/${contract.id} stream`,
      );
      assert.match(
      ...
      if (contract.stream_protocol === "openai_responses") {
        for (
          ...
          assert.match(
            streamBody,
            new RegExp(`event: ${event.replaceAll(".", "\\.")}`),
          );
        }
      }
      assert.equal(
      ...
      const interrupted = await fetch(
        interruptedRequest.url,
        interruptedRequest.init,
      );
      assert.equal(
        interrupted.status,
        200,
        `${provider.provider_driver}/${contract.id} interrupted stream`,
      );
      ...
      const terminalMarker = contract.stream_protocol === "openai_responses"
        ...
        : "[DONE]";
      assert.equal(
        interruptedBody.includes(terminalMarker),
        false,
        `${contract.id} interrupted terminal marker`,
      );
    }
  }
});
...
test("T1.5 async lifecycle validates and captures official poll/result wire", async (context) => {
  ...
  assert.deepEqual(
    validateProviderAuxiliaryRequest(contract, {
      method: "GET",
  ...
  assert.equal(
    (await fetch(`${baseUrl}/fal-ai/esrgan/requests/fal_mock_1/status`, {
      headers,
    })).status,
    200,
  );
  assert.equal(
    (await fetch(`${baseUrl}/fal-ai/esrgan/requests/fal_mock_1`, { headers }))
      .status,
    200,
  );
  assert.equal(
    (await fetch(`${baseUrl}/fal-ai/esrgan/requests/fal_mock_1/response`, {
      headers,
    })).status,
    200,
  );
  ...
  assert.deepEqual(audit.requests.map((request) => request.async_step), [
    "poll",
    "result",
    "result",
  ]);
  assert.ok(
    audit.requests.every((request) => request.validation_errors.length === 0),
  );
});
...
test("T1.5 Provider mock completes every declared async lifecycle", async (context) => {
  ...
  assert.ok(address && typeof address === "object");
  const baseUrl = `http://127.0.0.1:${address.port}`;
  const asyncContracts = catalog.providers.flatMap((provider) =>
    provider.contracts
  ...
  for (const { provider, contract } of asyncContracts) {
    ...
    const submit = t15ProviderRequest(
      baseUrl,
      contract,
      provider.test_model_ids[contract.api_types[0]],
    );
    assert.equal(
      (await fetch(submit.url, submit.init)).status,
      200,
      `${contract.id} submit`,
    );
    for (const step of contract.async_steps ?? []) {
      ...
      for (const field of step.required_query_fields ?? []) {
        url.searchParams.set(
          field,
          field === "task_id" ? "minimax_video_mock_1" : "minimax_file_mock_1",
        );
      }
      ...
      assert.ok(
        ...
        `${contract.id}/${step.name}: ${response.status} ${await response
          .clone().text()}`,
      );
      if (
        step.name !== "result" ||
      ...
      assert.equal(
        serialized.includes("http://mock"),
        false,
        `${contract.id} leaked placeholder artifact authority`,
      );
      const artifactUrl = serialized.match(
    ...
    assert.deepEqual(
      new Set(
        audit.requests.map((request) => request.async_step).filter(Boolean),
      ),
...
test("T1.5 manifest owns Provider normal, streaming, async, error, and variant cells", async () => {
  ...
  assert.ok(
    manifest.filter((item) =>
      item.mock_scenario === "stream_success" ||
      item.mock_scenario === "stream_interrupted"
    ).every((item) => item.execution_mode === "stream"),
  );
  assert.ok(
    manifest.filter((item) =>
      item.mock_scenario !== "stream_success" &&
      item.mock_scenario !== "stream_interrupted"
    ).every((item) => item.execution_mode === "immediate"),
  );
  ...
  assert.ok(
    manifest.some((item) =>
      item.case_id === "t1.5.openai.openai.responses.v1.llm.history"
    ),
  );
  assert.ok(
    manifest.some((item) =>
      item.case_id === "t1.5.openai.openai.responses.v1.llm.tool-history"
    ),
  );
  assert.ok(
    manifest.some((item) =>
  ...
  assert.ok(
    manifest.some((item) =>
      item.case_id ===
        "t1.5.claude.anthropic.messages.2023-06-01.llm.tool-history"
    ),
  );
  assert.ok(
    manifest.some((item) =>
  ...
  assert.ok(
    switchCases.some((item) =>
      item.switch_source_provider_driver === "openai" &&
      item.provider_driver === "google-gemini"
    ),
  );
  assert.ok(
    switchCases.some((item) =>
      item.switch_source_provider_driver === "google-gemini" &&
      item.provider_driver === "claude"
    ),
  );
  assert.ok(
    switchCases.some((item) =>
      item.switch_source_provider_driver === "claude" &&
      item.provider_driver === "openrouter"
    ),
  );
  assert.ok(
    switchCases.every((item) =>
      item.session === "shared-provider-switch" &&
      item.api_type === "llm" &&
      item.switch_source_provider_driver !== item.provider_driver
    ),
  );
  assert.ok(
    manifest.some((item) =>
      item.tags.includes("task_result_artifact") &&
      item.provider_driver === "openai"
    ),
  );
  assert.ok(
  ...
  assert.ok(
    manifest.some((item) =>
      item.expected_error_class === "provider_protocol_failed"
    ),
  );
  assert.ok(
    manifest.filter((item) => item.tags.includes("official_error"))
      .every((item) =>
        typeof item.expected_retriable === "boolean" &&
        !("expected_retryable" in item)
      ),
  );
  assert.ok(manifest.some((item) => item.tags.includes("variant")));
  assert.ok(
    manifest.every((item) =>
      item.layer === "T1.5" && item.semantic_rubric.length === 0
    ),
  );
  assert.ok(
    buildStaticManifest().every((item) =>
...
test("T1.5 variants are independently derived from official expectations", async () => {
  ...
  const inventory: ProviderInventory = {
    ...
    models: [
      {
        exact_model: "gpt-5.5@openai-t15",
  ...
  assert.deepEqual(
    cells.find((cell) =>
      cell.model.provider_model_id.endsWith("reasoning-high")
  ...
  assert.throws(
    () =>
      variantCells(catalog, {
        ...inventory,
        models: inventory.models.slice(0, -1),
      }),
...
test("T1.5 typed request fixtures use current provider-neutral methods without legacy envelopes", () => {
  ...
  const streamParams = buildT15TypedParams(
    "llm",
    "gpt-5.4@t15-openai",
    "run",
    "stream",
...
test("T1.5 success mapping requires canonical output, usage, and async attribution", async () => {
  ...
  assert.throws(() =>
    assertT15ResponseMapping("video.txt2video", {
      status: "succeeded",
...
test("T2 excludes independently callable metadata variants", async () => {
  const inventories: ProviderInventory[] = [{
    ...
    models: [
      {
        exact_model: "gpt-5.4@openai-main",
      ...
      {
        exact_model: "gpt-5.4:reasoning-high@openai-main",
        provider_model_id: "gpt-5.4:reasoning-high",
        provider_actual_model_id: "gpt-5.4",
        api_types: ["llm"],
        logical_mounts: [],
...
test("report redaction removes secrets and totals statuses", () => {
  ...
  assert.equal(
    isProviderRestricted(new Error("request not allowed for this model")),
    true,
  );
  assert.equal(
...
test("report schema rejects version drift and duplicate case ids", () => {
  ...
  assert.throws(
    () =>
      validateAcceptanceReport({
        ...report,
        limits: { ...report.limits, provider_concurrency: -1 },
      }),
...
test("named artifact validation reads and verifies ZIP entries", async () => {
  const bytes = new Uint8Array(
    await readFile(
      join(here, "../../jarvis_media_dv/assets/archive_mixed.zip"),
    ),
  );
...
test("legacy DOC and PPT fixtures are genuine OLE Office binaries", async () => {
  for (
    ...
    assert.ok(
      bytes.includes(Buffer.from(stream, "utf16le")),
      `${name} is missing ${stream} OLE stream`,
    );
  }
  assert.ok(
    (await readFile(join(here, "../fixtures/facts.doc"))).includes(
      "I am a test document",
    ),
  );
  assert.ok(
    (await readFile(join(here, "../fixtures/facts.ppt"))).includes(
      "AICC-FIXTURE-7319",
    ),
  );
});
...
test("artifact validation records media metadata", async () => {
  const png = new Uint8Array(
    await readFile(join(here, "../fixtures/mask.png")),
  );
  const pngAudit = await validateArtifactBytes(png, {
    id: "inline",
    label: "image/png",
  });
  ...
  const transparent = new Uint8Array(
    await readFile(join(here, "../fixtures/transparent.png")),
  );
  const transparentAudit = await validateArtifactBytes(transparent, {
    id: "inline",
    label: "image/png",
  });
  ...
  assert.doesNotThrow(() =>
    assertBackgroundRemovalTransparency([transparentAudit])
  );
  assert.throws(() =>
    assertBackgroundRemovalTransparency([{
      ...transparentAudit,
  ...
  const wav = new Uint8Array(
    await readFile(join(here, "../../jarvis_media_dv/assets/audio_speech.wav")),
  );
  const wavAudit = await validateArtifactBytes(wav, {
    id: "inline",
    label: "audio/wav",
  });
