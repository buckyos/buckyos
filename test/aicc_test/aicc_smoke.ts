...
type SmokeCase = {
  ...
  defaultAlias?: string;
  buildPayload: (runId: string) => SmokePayload;
  check: SummaryCheck;
};
...
const inpaintImageResource: ResourceRef = {
  kind: "base64",
  mime: "image/png",
  data_base64:
    "",
};
...
const allCases: SmokeCase[] = [
  ...
  },
  {
    ...
    buildPayload: () => ({
      ...
      resources: [inpaintImageResource, imageMaskResource],
    }),
  },
  {
...
function buildMethodPayload(
  ...
  const payload = testCase.buildPayload(runId);
  return {
...
async function runCase(args: {
  ...
  const requestPayload = buildMethodPayload(testCase, caseRunId, modelAlias);
...
function selectedCases(): SmokeCase[] {
  ...
  return allCases.filter((testCase) => allowed.has(testCase.method));
}
