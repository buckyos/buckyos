...
TEST_TARGET_DIR="${REPO_ROOT}/src/target/test_klog_gateway_abnormal_dv"

cd "${REPO_ROOT}"

export BUCKYOS_ROOT="${BUCKYOS_ROOT:-${REPO_ROOT}/.dev_buckyos}"
export KLOG_CLUSTER_DV_MODE="local-gateway-abnormal"
export KLOG_CLUSTER_DV_ROUTE_MODE="target-gateway"

if [[ -z "${KLOG_DAEMON_BIN:-}" ]]; then
  echo "[build] cargo build -p klog_daemon"
  (cd "${REPO_ROOT}/src" && OPENSSL_NO_VENDOR="${OPENSSL_NO_VENDOR:-1}" cargo build -p klog_daemon)
