...
| `server-runner` | HTTP 服务器框架 | `Runner::new(port)`, `runner.add_http_server(path, server)`, `runner.run()` |
...
pub async fn run_service() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 9. 启动 Runner
    let mut runner = server_runner::Runner::new(SERVICE_PORT);
